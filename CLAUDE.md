...
- ★`lib/palette.ts` — **有彩色の「役 → 色」の対応表**（第73巡）。分類に使う色は
  **9つだけ** … ドメイン4（券・ブリーフ・バインダー・ストック・マップ）＋
  ...
  ★★★**状態の色をそのまま文字に使わないこと**（第76巡）。紙の上で本文の 4.5 を
  満たすのは `RUST` と `INK` だけ。`GREEN`/`BLUE` は**面**として使い、載せる色は
  `GREEN_INK`/`BLUE_INK` から取る。
...
- ★★★`lib/printGrain.ts`（券・CSS の面）と `lib/paperTexture.ts`（図形・canvas に
  焼き込む）— **質感の2つの入口。素材は同じ 1 枚**（`public/halftone.webp`。第76巡に
  ユーザー指定「両方とも網点」。「券は板紙・図形は切った紙。混ぜない」は**撤回**）。
  作り方は `tools/make-halftone.mjs`（元画像はリポジトリに無い。引数で渡す）。
  ★★★**テクスチャは明暗だけを足すもので、色を変えるものではない。**
  タイルは**平均 128 の無彩色**で、**128 は `soft-light` の恒等点**だから地の色が
  原理的に動かない。★★**`multiply` を使わないこと** ―― 原理的に暗くしかできず、
  質感だけを乗せられない（実測 142,884 画素で ΔE 0.135・彩度 −20%。`soft-light`
  は ΔE 0.001）。★★**1画像画素 = 1デバイス画素**（`background-size: 126 / dpr`）。
  ★継ぎ目は**消すのではなく最初から作らない**（網点の周期 9px × 14 ＝ 126px）。
  ...
  ★焼き込みは `lib/solidPaint.ts` が絵を焼くときに1度だけ（毎フレームの負荷はゼロ、
  図形が回れば質感も回る）。絵ごとに**4向き×ずらし**が当たる。
  ★`paper-kraft.webp` / `make-paper.py` / `print-grain.webp` / `make-grain.mjs` は
  **まだ消していない**（テクスチャを試している最中）。決まったら4つまとめて消す。
