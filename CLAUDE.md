...
- `components/tabs/RecordTab.tsx` / `JournalTab.tsx` — レコード / 今日・アーカイブ。
  ...
  ★★★**組み立ての順は「キーの列 → 下の段 → 本体 → 円」**（第95巡に逆転）。
  **キーの列を動かさない**（`NAV_H` に追従している）。**下の段のほうが行く。**
  ★★★**腰（砂時計の締まり）は式で縛ってある** ―― `DIAL_CX` は
  `(DIAL_RATIO − (1 − DIAL_WAIST)) / 2` から導く。**直径だけ動かすと腰が広がる**
  事故が起こせない。実測 … 腰は 117px のまま（＝波形はそのまま）。
  ★**本体は左右へはみ出したまま**（685.7px ＝ 片側 147.8px。ユーザー確定）。
  ★★★**下の段がそのままキーの穴**（キーは `well="transparent"`）。ラベルは
  `bodyInkOn(下の段)`＝紙色。**面が地から墨へ移ったので、地から決めない。**
