...
- ★★★`components/home/bandMotion.ts` — **帯の「物理っぽい動き」はここ1つ**
  （第109巡に新設・**第110巡に作り直し**）。④a 止まるときの慣性／
  ① 引き抜いた瞬間の**穴と追突と流れの再開**／② 戻すときの**指に付いてくる隙間**／
  ⑤ 挿し込まれたときの**バウンド**。
  ★★**`pullBus` と同じ作法**（module のただ1つの入れ物 ＋ 段ごとの rAF）。
  ★★★**ずれは `transform` だけ**（レイアウトを 1px も動かさない）―― 帯の継ぎ目の
  無さは「2周の幅が厳密に等しい」ことだけで成り立っている。
  ★★**ピルは2周ぶん DOM に居る**ので `data-pill-id` で引いて**両方に同じ値**。
  ★★`.band-track` の `transform` は**流れの WAAPI が占有している**ので、帯ぜんたいの
  ずれは**1枚外の `.band-shift`**、ピルごとのずれは**`.band-slot`（ピルの外の包み）**
  が持つ ―― ★★★**ピル自身の `transform` に混ぜない**（押下の縮みと 420ms の
  transition が同居しているので、毎フレームの書き込みと CSS の補間が二重に効く）。
  ★★★**ずれには行き先が要る**（`Off = Spring & { hole, gap, wait }`。足し算で持つ）。
  **頭打ちは帯ぜんたいのずれだけ** ―― ピルごとに `NUDGE_MAX`(24) を掛けていた
  第109巡は、**ピル1枚ぶん（120〜250px）の8割を切り捨てていた**。
  ★★★**バネは役で分ける**（第111巡にユーザー確定「**跳ね返りは全部やめる／再開の
  ときだけ軽い慣性**」）… **ピルごとのずれ＝`K_SETTLE`（行き過ぎない）／帯ぜんたいの
  一発＝`K_SWING`**。★**`K_CATCH` にしない**（4フレームで着くので 200px が瞬間移動
  になる。実測 1フレームで 232px）。
  ★★★**隙間も穴も「片側だけ ±w」**（挿し口より後ろを丸ごと動かす）―― **両側 ±w/2 は
  受け渡しで左右が新しいピルの上へ乗る**（第110巡の「どこかに重なる」の正体）。
  ★★★**隙間は「指が指している所」に開き、並びのほうが合わせる**（第112巡に
  ユーザー確定「**順番がいくら入れ替わっても問題ない。どんな時でも、任意のピルと
  ピルの間に戻せるように**」）。★★★**第111巡は逆だった**（並びが場所を決める）ので、
  **指をどこへ持っていっても1か所にしか戻らず、そこ以外では隙間も開かなかった**。
  ...
  ★★★**穴を仕込むのは「弾けた瞬間」**（`armed`。`taken` では早すぎ、`items` から
  消えたあとでは**もう瞬間移動を終えている**）。
  ★★★**流れの再開と慣性の一発は、穴より先に・無条件で**（第111巡）―― 第110巡は
