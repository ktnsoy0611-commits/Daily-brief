...
- ★★★**帯のピルを引いて日付を割り当てる**（第102巡にユーザー指定。仕様の正は
  ...
  **進みは時間ではなく指が引いた距離**なので、曲線4本は使わない。
  ★★★**手つきは「ゴム → ばちん → 段が弾む」**（第104巡にユーザー確定）…
  閾値までは**帯のピルそのもの（DOM）が縦に伸び**（`pullTaut`。上の縁を留めて下へ。
  ピルごと動かすとただの移動に見える）、越えた瞬間に**伸びを解いて消え、幽霊が指
  そのものへ出て弾む**（`SNAP_POP` ＋ `haptic`）、以後**段が 1→2→3 とカクンと生える**
  ...
  **1秒に120歩＝物理が2倍速**になり、めり込みが倍になって床を突き抜ける
  → **実時間を貯めて `STEP_MS` の整数歩だけ**進める（`MAX_STEPS` で頭打ち）。
...
- `lib/spring.ts` — **canvas の図形だけの動きの土台（バネ＝減衰振動）**。係数は**3組**
  （運ぶ `K_TRAVEL`／吸い付く `K_SETTLE`／**揺れる `K_SWING`**。第103巡に追加）。
