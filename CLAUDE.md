...
- ★`components/explore/samples/` — **券の見本帳**（第80巡に**4案**へ絞った）。
  P1 天地／P2 逆天地／P3 重ね／P4 活字 ＋ `TicketParts.tsx` ＋ `index.ts`。
  ★★★**紙は白1色**。色が出るのは**大きな英語**（`poster` 38 / `black` 900 /
  ドメインの色）と**鋏痕**の2か所だけ（ユーザー指定「色は背景ではなくワンポイント」）。
