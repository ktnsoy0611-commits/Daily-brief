...
- ★★★`components/home/bandMotion.ts` — **帯の「物理っぽい動き」はここ1つ**（第109巡）。
  ④a 止まるときの慣性／④b ピルが抜けたあとの**追突**／⑤ 戻したときの**左右の弾き**。
  ...
  **1枚外の `.band-shift`** が持つ。★上限 `NUDGE_MAX`(24)、バネは `K_SWING` 1組。
  ★★★**帯は `items` の同一性が変わるたび `cancel()` され、先頭へ飛んでいた**
  （第109巡に根治）。引き金を**中身の署名**にし、**位相を割合で引き継ぐ**。
