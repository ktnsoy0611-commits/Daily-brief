...
- `components/tabs/RecordTab.tsx` / `JournalTab.tsx` — レコード / 今日・アーカイブ。
  ★★★**大きな円は `JOURNAL_FACE`（アプリのメインカラー）**（第93巡にユーザー
  指定。明るいタブも暗いオーバーレイも**同じ色**で、画面ごとの反転はやめた）。
  ★★縁の目盛りは `bodyInkOn(円)` の**ベタ**（比 7.12。**半透明にすると 1.94 で
  消える**）。★★★**寸法は `lib/dial.ts` の1か所** ―― 同じ円をホームの山が
  **canvas** で描くので、数を2度書くと必ず食い違う。
  ★実測 … 円と地の比はクリーム **2.02**／墨 **5.9**（元の黒い円は 11.25）。
...
- ★★★`lib/dial.ts` — **録音のダイヤル（円＋縁の目盛り5本）の寸法はここ1つ**
  （第93巡）。`VoiceStudio` の **SVG** とホームの山の **canvas** が同じ数を読む。
