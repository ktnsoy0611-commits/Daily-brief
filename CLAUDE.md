...
- ★★★**帯のピルを引いて日付を割り当てる**（第102巡にユーザー指定。仕様の正は
  ...
  ★★★**摘みは** `PULL_ARM`(**44** ＝ ピル1つぶんの高さ) / `PULL_RESIST`(0.75) /
  `PULL_GIVE`(1.6)。**ゴムの側は指が引いた距離**なので曲線4本は使わない。
  ★★★**弾けたあとだけは時間**（`MORPH_STEPS` ＝ `T_ITEM` 417ms。3段なら
  0/140/280ms で「ぽこ・ぽこ・ぽこ」）。**`MORPH_SPAN`（距離）は第106巡に削除。
  復活させない**（ユーザー「**距離によって変わってるのは不自然**」）。
