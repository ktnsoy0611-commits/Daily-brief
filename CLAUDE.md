...
- ★`components/explore/samples/` — **券の見本帳**（第79巡に**9案**）。
  N1 帯／N2 全面／N3 額／N4 札／N5 挟み／N6 柱／N7 札／N8 半券／N9 見出し
  ＋ `TicketParts.tsx` ＋ `index.ts`。★★**2つの軸**で振ってある ――
  **N1〜N5＝色がどれだけ見えるか**（N2→N1→N5→N3→N4 の順に増える）、
  **N6〜N9＝面をどう割るか**（縦に割る／重ねる／切り離す／上下を入れ替える）。
  ★★見本帳は**案ごとに違う紙の色**で並ぶ（`DevStageTab` の `SAMPLES` 4枚を巡らせる）。
...
- ★`lib/palette.ts` — **有彩色の「役 → 色」の対応表**（第73巡）。
  ★★★**メイン5色 ＋ 予備4色**（第77巡）。**図形5 も 券の紙4 も、メインから取る**
  （TASK と EXPLORE は別のアプリで同じ画面に並ばないので重なってよい）。
  ★★色そのものは `SCHEME`（`lib/constants.ts`）。**パレットを替えるときは
  `SCHEME` だけ書き換える。** ★kind（10種）は色を持たない ―― ドメインを通す。
  ★★★**メイン5色は「色相の覆い・明度の幅・彩度の幅・見分け」の4つで採点して選ぶ。**
  **見分けだけを最大化しないこと** ―― 第76巡はそれで赤が2つになり、盤の雰囲気が
  図形に出なかった。★多いタグ（work/life/growth）は先に離す。
  ★★★**状態の色をそのまま文字に使わないこと**。紙の上で本文の 4.5 を満たすのは
  `RUST` と `INK` だけ。`GREEN`/`BLUE` は**面**で、載せる色は `GREEN_INK`/`BLUE_INK`。
