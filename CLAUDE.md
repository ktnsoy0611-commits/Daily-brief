...
- `components/tabs/RecordTab.tsx` / `JournalTab.tsx` — レコード / 今日・アーカイブ。
  ★★★**録音の画面は「カセットをごく近くで見た絵」**（第94巡にユーザー指定）。
  **大きな円2つは墨（`INK`）**、その**後ろに青い本体**（`JOURNAL_FACE`）。
  ★★★**円の大きさと位置は絶対に動かさない**（ユーザー確定）。本体は円の
  **兄弟**として `zIndex: 0`・`pointerEvents: "none"` で置く ―― 中に入れると
  `.vs-reel-spin` で一緒に回る。掴めるのは円だけ（`dialAt` は円の式）。
  ★★**本体の四隅は画面の外**（円の直径が画面幅の 1.3 倍）。逃げ道は無い。
  はみ出す量は `PLATE_PAD` の1つ。**アイコンの余白の比は使えない**（当てると
  本体が 1224px 高になり全面青になる）。
  ...
  **目盛りの寸法は `lib/dial.ts`**。
  ...
  ★キーの面は両方の画面で白。輪郭は**黒い穴**（`STUDIO.well`）が作る。
  ★キーの記号は**面から導く**（`bodyInkOn` / `redOn` / `STUDIO_KEY`）。
...
- ★★★`lib/cassette.ts` — **カセット（JOURNAL の顔）の寸法はここ1つ**（第94巡）。
  ...
  リールと帯＝黒**。★★**ただしタブバーの中だけは1色**（地が選択状態で変わるので
  色を決め打ちできない）。
- ★★★`lib/cardShape.ts` — **BRIEF の札の写真を切り抜く4つの形**（第94巡）。
  ★★★**対応表を手で書かない** ―― 券の鋏痕（`lib/ticket.ts` の `PUNCH_BY_DOMAIN`）
  から `SHAPE_BY_PUNCH` で導く。弧＝バショ／斜め＝タイケン／直角＝ジョウホウ／
  切れ込み＝モノ。★マスクは **SVG の `data:` URI 1枚 ＋ `maskSize: "100% 100%"`**
  （層を重ねないので `intersect` の罠が無い）。`maskImage` と `WebkitMaskImage` は対で。
