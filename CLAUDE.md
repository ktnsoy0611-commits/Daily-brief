...
- ★★★`components/home/bandMotion.ts` — **帯の「物理っぽい動き」はここ1つ**
  ...
  **頭打ちは帯ぜんたいのずれだけ** ―― ピルごとに `NUDGE_MAX`(24) を掛けていた
  ...
  ★★★**隙間は「本当に入る場所」に開く**（第111巡にユーザー確定）―― 挿し込み位置は
  `bandRows` の順で決まるので、**指の位置とは一致しようがない**。index は落とす前に
  計算できる（`lib/homeBand.ts` の `somedaySlotOf`）。
...
- ★★★`lib/homeBand.ts` — **帯の中身と並びの規則の正**。
  ...
  **`keepId` は切り落とさない**（第111巡）―― 帯へ戻したタスクは `appState.tasks` の
  ...
  ★★★**`somedaySlotOf(state, taskId)`** … そのタスクを戻したとき**下の段の何番目に
  入るか**。**落とす前に計算できる**ので、帯の隙間はここから開ける。
