...
- 確定ビュー(`ConfirmedStack`)側の下部固定ボタンは`handleRegister`→
  ...
  作業していたことが伺える。
- **対応**: `registerBinder`側のボタン文言を「今日を登録する」に変更し、
  選択確定の「バインド！」と明確に区別した。ロジックは変更していない
  (仕様通りの「未実行はcandidateへ戻す」動作はそのまま)。
...
- **あわせて実施したUI再設計**: `ExecuteTab.tsx`の「← バインダーに戻る」
  ...
  同じ「PAPER地+SOFT_SHADOWで浮く丸チップ」語彙のボタンへ作り直し、
  Masthead込みで`position:sticky; top:0`の専用ヘッダー枠にまとめた
  (`backChipStyle`)。html/body自体のスクロールを止めたことが本質的な
  対応だが、§7.5点3で一度「外側のロックが別コードに上書きされて壊れる」
  regressionが実際に起きている経緯を踏まえ、同種の再発(`data-tab-scroll-root`
  自体が意図せずスクロール可能に戻るケース)に対する保険として、このヘッダー
  自身もその親を基準にsticky化した(ただしhtml/body自体が漏れて
  スクロールする今回のバグ自体への効果はない。sticky位置の基準は
  `data-tab-scroll-root`になるため)。
- **未検証事項**: この2点はいずれもWebKit実機での直接確認ができておらず
  (環境の制約は§7と同じ)、コード上の根本原因の特定とChromiumでの間接検証
  (localStorageの状態遷移・ボタン文言・sticky構造の確認)まで。次回は
  実機での再テストが必須。
