...
- **WebKitで検証していないこと自体が穴。** Playwrightのwebkitエンジンで再現を試みる
  ...
  リモートインスペクタ、または画面録画をユーザーにもらう）。
- フォント(Zen Kaku Gothic New, next/font)のロード完了タイミング: 実測(ResizeObserver)や
  クランプ計算(em)がフォントスワップ前後で変わり、遅れて再レイアウトされる可能性。
- iOS SafariのURLバー伸縮によるビューポート高の動的変化と実測の発火タイミング。
- 初回レンダーは`cardBox`未確定のフォールバック幅(min(88vw,340px)+aspectRatio)で
  描画され、実測確定時に1回スナップする（ユーザーのスクショの大きすぎるカードは
  このフォールバック描画に見える。**Safariで実測が一度も効いていない疑い**:
  祖先flexチェーンの高さ解決がSafariで失敗し arenaRef の rect.height が想定外の値/0に
  なっているケースを最優先で疑うこと）。
