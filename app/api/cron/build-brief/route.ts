...
// ★★★**ここが「実際に捨てる」への切り替え地点**（2026-09-11）。
// いま画面の側は `lib/homeBand.ts` の `BRIEF_POOL_CAP`(15) で**見せる枚数だけ**
// 絞っている（ユーザー確定「表示だけ15に見せる」）。裏のストックは減らないので、
// **未消化が 40 に達している間は生成が止まったまま**になる。
// 新しい提案が来なくなったら、ここを `BRIEF_POOL_CAP` にし、下の号の書き戻し
// （`decks[editionKey] = ...` のところ）で**古い未読を上限まで落とす**こと。
// ★`generatedDecks` はサーバーのもの（`lib/dataStore.ts` の `SERVER_OWNED_KEYS`）
//   なので、**本当に減らせるのはここだけ**。画面側で消しても次の読み込みで戻る。
// ★★なお実測で 41 になるのは、情報源の提案カードが `genCount` の枠の**外**で
//   `unshift` されるため（下の `sourceProposal`）。上限＋1 まで伸びる。
