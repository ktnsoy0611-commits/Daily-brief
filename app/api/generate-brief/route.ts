...
const PAGE_TEXT_LIMIT = 5000; // 1ページあたりプロンプトへ入れる本文の最大文字数
const LINKS_LIMIT = 60;       // 1回目に渡す実在リンクの最大数
...
async function fetchPage(url: string): Promise<FetchedPage> {
  try {
    ...
    return { url: res.url || url, ok: true, text: htmlToText(html), html };
  } catch {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (linkPool.length > 0) {
      ...
      const indexText = readable.map((p) => p.text.slice(0, 2500)).join("\n---\n");
      const discoverUser = `以下の情報源から、【ユーザーの興味・好み】に合致する個別記事のURLを最大${count}件抽出してください。出力は抽出したURLのみを改行区切りで出力し、前後に説明文やコードフェンスは含めないでください。「実在リンク一覧」に無いURLは絶対に出力しないでください。
...
${listText}`;
    ...
    const pageBlocks = usable
      .map((p) => `URL: ${p.url}\n内容: ${p.text.slice(0, PAGE_TEXT_LIMIT)}`)
      .join("\n\n====\n\n");
