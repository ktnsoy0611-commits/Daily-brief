...
export const metadata: Metadata = {
  ...
  appleWebApp: {
    capable: true,
    // ★★★`black-translucent` に戻した(2026-08-19・第29巡)。
    //   一度 `default` を試したが、**`default`/`black` は「ステータスバーの
    //   見た目だけ変える設定」ではなかった** — iOS はステータスバーぶんを
    //   web ビューの外(OS が別に確保する帯)として扱うようになり、そこが
    //   黒い帯として残り、しかも中身全体がそのぶん下へ押し出されて
    //   タブバーの位置までずれた(実機で「上に変な黒い帯」「タブバーが
    //   下寄りすぎ」と報告)。`black-translucent` は**中身が画面の裏側まで
    //   1枚で伸び**、ステータスバーはその上に透けて重なるだけなので、
    //   帯もずれも起きない。時計とアイコンが白くなって明るい地色の上で
    //   読みにくいのは既知のトレードオフ(iOS の3択にこれ以外の選択肢が無い)。
