...
import { BLUE, INK, MUTED, PAPER, RUST, SANS, itemKindOf, SECOND } from "@/lib/constants";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: SPACE.lg, padding: `${SPACE.xl}px 0 ${SPACE.md}px` }}>
      ...
      <div style={{ position: "relative", width: 200, height: 28, overflow: "hidden" }}>
        ...
        <span className="brief-roll-shape" style={{ ...base, marginTop: -7 /* ★目盛りの外（図形の -height/2＝縦の中央合わせ） */, width: 0, height: 0, borderLeft: "8px solid transparent", borderRight: "8px solid transparent", borderBottom: `14px solid ${INK}`, animationDelay: "1.2s" }} />
        <span className="brief-roll-shape" style={{ ...base, marginTop: -5 /* ★目盛りの外（図形の -height/2＝縦の中央合わせ） */, width: 20, height: 10, borderRadius: RADIUS.sm, background: RUST, animationDelay: "1.8s" }} />
      </div>
