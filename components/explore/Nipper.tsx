...
const GRIP: Station[] = [
  // ★**左の縁はまっすぐ**（`cx − hw` を −10 で通す）。ふくらむのは右だけ。両側に
  //   ふくらませると道具ではなく紡錘に見える。段は**右にだけ**付く（頭は柄より細い）。
  // ★**柄は薄く、頭だけ厚い**。柄まで厚くすると、一点透視のずれで左の側面が広く出て
  //   **白い帯が縦に走る**（参考の道具では明るい左の縁は幅の1割ほど）。厚みが要るのは
  //   頭だけ ― そこだけ**天面**が見えて塊として立つ。
  // ★頭の天は**奥の受けより高い**。一点透視で手前ほど下がるので、物理的に高くして
  //   おかないと、見た目では手前の頭が奥より低く沈む（11巡目に実測）。
  [-700, 15, 7, 20],     // 先。★幅を残したまま丸く終わる（尖らせない）
  [-682, 26, 10, 24],
  [-650, 34, 12, 26],
  [-560, 42, 13, 32],
  [-452, 48, 15, 38],
  [-380, 50, 15, 40],    // 腹。いちばん太い（全長の 6 割ほど下）
  [-280, 46, 15, 36],
  [-222, 39, 14, 29],
  [-220, 30, 22, 20],    // ★頭。**柄より細い**（右にだけ段が付く）
  [-10, 30, 22, 20],
  [-8, 22, 19, 12],      // ★右上の矩形の欠き（参考画像2の段）
  [26, 22, 17, 12],      // 天
];
/**
 * 固定部の本体（奥・左）。★**手前の柄より薄く**、**手前の柄より低い位置で始まり、
 * 高い位置で終わる**。厚くすると左の側面が広く出て、奥の板が手前より明るくなる。
 * ★左の縁はまっすぐ（−68）。★★右の縁は手前の柄の左の縁より**内側まで伸ばす**（0）―
...
const LEVER: Station[] = [
  ...
  [-300, 34, 11, -34],
  [-222, 34, 11, -34],
  [-220, 34, 13, -34],   // ★頭。ここで厚くなる（受けの塊）
  [-16, 34, 13, -34],
  [-2, 30, 11, -38],
];
/** ダイのスリット。★受けの面に開いた縦の溝。`invert` で凹みにする。 */
const SLOT = { x0: -58, x1: -39, y0: -177, y1: -49, z0: Z_FAR - 8, z1: Z_FAR + 14 };
