...
const CALM = 0.05;
/**
 * ★★★**挿し口を乗り換えるのに要る「得」（px）**（2026-09-17・第118巡）。
 *
 * ★★★**遊びが1つも無かった**のが「③ ガクガク震えてチカチカ点滅する／全然
 *   アニメーションしていない／重なる／位置が定まらない」の真因。
 *   `at` は段の rAF が**毎フレーム矩形から選び直す**ので、**境目の近くでは
 *   1px の揺れで隣へ飛ぶ**。飛ぶたびに `bandGapAt` が `m.open.p = 0` へ戻すため、
 *   **隙間は毎フレーム開き直して 1px も育たない**（＝何も動いていないように見える）。
 * ★★`tTo` の側には第109巡から `RAIL_HYST`(22) が在ったのに、**ここだけ素の
 *   比較のままだった**。同じ作法を当てる。
 * ★目盛りの外（手ざわり）。
 */
export const GAP_HYST = 22;
...
/**
 * ★★★**まだ閉じかけの隙間が残っているか**（2026-09-17・第118巡）。
 * ★★**受け皿（`shut`）は1本しか無い**ので、閉じ切る前に次の乗り換えが来ると
 *   **上書きされて一度も閉じ切らない**（開いた隙間が居座って**重なる**）。
