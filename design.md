...
図形に出なかった。★**多い3つ（work / life / growth）は先に離す**（最小 ΔE 0.34）。

| 役 | メイン | サブ | サブ比 | 本文比 |
|---|---|---|---|---|
| WORK ／ タイケン ／ **選ばれている** | `#FF582E` | `#FFFBF5` | 3.05 | 4.73 |
| LIFE ／ モノ ／ **肯定** | `#E5F167` | `#3E0D20` | 13.33 | 12.10 |
| GROWTH | `#3E0D20` | `#D1EAF1` | 13.05 | — |
| WELLNESS ／ ジョウホウ | `#D1EAF1` | `#9C0003` | 6.94 | 11.85 |
| SOCIAL ／ バショ | `#FFBF85` | `#3E0D20` | 10.16 | 9.23 |
...
- ★**状態の3色は9つから借りる**。新しい色を足さない（肯定＝若草・選択＝朱は
  メインから、危険＝深紅は予備から）。
- ★地は `BD_GREY`(`#FFFBF5`) と `JOURNAL_BG`(**`#2C2627`**) の2つだけ。
  ★★★**JOURNAL だけ暗いアプリ**（第77巡）。**地の上に直接いるもの**は色を
  ハードコードせず、`AppShell` が置く **`--ink-on` / `--on-ink` / `--muted-on`** を読む
  （`lib/palette.ts` の `inkVarsOn`。中身は `bodyInkOn` が導く）。
  ★カードの中は `PAPER` なので、そのままで読める ―― 直すのは地に直接いるものだけ。
