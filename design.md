...
| 名前 | 値 | 比 | 役 |
|---|---|---|---|
| `INK` | `#3E0D20` | 15.9:1 | 主役の文字。★盤の BLACK CHERRY（MAIN/TEXT） |
| `CHARCOAL` | `#2A2A28` | 12.6:1 | ほぼ黒 |
| `SECOND` | `#5A5A54` | 6.1:1 | 副文（二番目に強い文字） |
| `MUTED` | `#8E8E88` | 2.9:1 | 補助・非活性。★**本文に使わない**（AA 4.5:1 を割る） |

★★**分類に使う有彩色は9つだけ**（第73巡に決め、第76巡に色を差し替えた）。
色そのものは `lib/constants.ts` の **`SCHEME`**、「役 → 色」は **`lib/palette.ts`** の
1か所。**この2つ以外に色を書かない。**

★★★**色は「メイン × サブ」の組で持つ**。**メイン**＝面の色（普通の意味の
カラースキーム）／**サブ**＝その面に載る**大きな文字**の色。★★**サブは大きな
文字だけ**（券の題 `head`/900・図形の名前・罫）。本文（`body` 13）と小さなラベルは
`bodyInkOn()` が `INK`／`PAPER` から導く ―― **載せる大きさで要る比が変わる**
（WCAG … 本文 4.5／18.66px 太字以上と図 3.0）。

| 役 | メイン | サブ | サブ比 | 本文比 |
|---|---|---|---|---|
| ドメイン バショ | `#FFBF85` | `#3E0D20` | 10.16 | 10.2 |
| ドメイン タイケン | `#FFA5CB` | `#9C0003` | 4.76 | 9.0 |
| ドメイン ジョウホウ | `#D1EAF1` | `#9C0003` | 6.94 | 13.1 |
| ドメイン モノ | `#EEDDFF` | `#9C0003` | 6.80 | 12.8 |
| タグ WORK ／ **選ばれている** | `#FF582E` | `#FFFBF5` | 3.05 | — |
| タグ LIFE ／ **肯定** | `#E5F167` | `#3E0D20` | 13.33 | — |
| タグ WELLNESS | `#C3D9FF` | `#9C0003` | 6.08 | — |
| タグ SOCIAL ／ **墨** | `#3E0D20` | `#D1EAF1` | 13.05 | — |
| タグ GROWTH ／ **危険** | `#9C0003` | `#D1EAF1` | 6.94 | — |

★★★**多いタグから先に離す**（第76巡にユーザー指定「work と life と growth が
多いので、そこら辺の色が被らないように、かつパレットの雰囲気をとらえた主要な色に」）。
その3つはこの盤で**彩度の高い上位3色**で、互いの見分けは最小 ΔE **0.25**、
図形5色ぜんぶでも **0.22**（第75巡は 0.116）。★**「前の色にいちばん近い」で
解かないこと** ―― 第75巡はそれで近い色どうしが山に並んだ。

★★★**状態の色を、そのまま文字に使わないこと。** 紙 `#FAFAF9` の上で本文の 4.5 を
満たすのは `RUST`(8.3) と `INK`(15.7) だけ。`GREEN`(1.2) と `BLUE`(3.1) は
**面**として使い、載せる色は `GREEN_INK` / `BLUE_INK` から取る。

- ★**状態の3色は9つから借りる**。新しい色を足さない。**ドメインの4色は
  どの状態とも重ならない**ので、券の色が UI の合図と混ざらない。
- ★地は `BD_GREY`(`#FFFBF5`) と `JOURNAL_BG`(`#D1EAF1`) の2つだけ。
...
- タイルは **平均 128 の無彩色**（`tools/make-halftone.mjs`）。128 は `soft-light` の
...
- ★★**`multiply` を使わない** ―― 原理的に**暗くしかできない**ので、質感だけを
  乗せられない。実測（142,884 画素）… `multiply` は ΔE 0.135・**彩度 −20%**、
  `soft-light` は **ΔE 0.001**・彩度そのまま。
- ★**1画像画素 = 1デバイス画素**で敷く（`background-size: タイル / dpr`）。
