...
★★★**2026-09-07 に地と墨が変わった**（`docs/home-spec.md` §2）。地 `#FCF7F4`／
墨 `#23252B`（どちらも参照画像の実測値）。★★**無彩色は地と `INK` の混色で導き、
比は前の組み合わせと同じ値に合わせてある** ―― 色みだけが寒色へ移り、明暗の階層は
1段も動いていない（移行の前後で computed style を 3566 件突き合わせ、寸法・文字の
差は 0 件）。

| 名前 | 値 | 比 | 役 |
|---|---|---|---|
| `INK` | `#23252B` | 14.4 | 主役の文字 |
| `CHARCOAL` | `#313238` | 12.0 | **全画面のオーバーレイの地**（第80巡に復活） |
| `SECOND` | `#5D5E61` | 6.1 | 副文（二番目に強い文字） |
| `MUTED` | `#8C8B8C` | 3.2 | 補助・非活性。★**本文に使わない**（4.5 を割る） |
| `SURFACE` | `#EAE6E3` | 1.17 | **面**（カード・入力欄・小区分）。第80巡に復活 |
| `TAG_STEPS` | 5段 | 14.0／7.7／3.9／2.2／1.3 | **タスクの図形の塗り**（L\* 等間隔）。★色ではない |

★暗い画面用の面は `SURFACE_DIM` `#404045`（地との比 1.24）／
`SURFACE_DIM_2` `#4E4D52`（カードの上で 1.24）。
★★★**半透明の重ねがけをやめるために面のグレーが在る** ―― 白の 8%/16% を重ねると
下に何が居るかで色が変わり、**重ねる順で結果が動く**。不透明の1枚なら動かない。
★黒は `#23252B` の1つだけ（純黒 `VOID` は穴の芯だけ）。`CHARCOAL` は**ほんの少し
だけ明るい**（地を黒そのものにすると、その上の墨のものが沈んで層が読めない）。

### 3-2. 有彩色 — **5色。役の階段であって、分類の色ではない**

★★★**色そのものは `lib/constants.ts` の `PALETTE`（5行）だけ**。役との対応は
すぐ下の `SCHEME` と `lib/palette.ts`。**パレットの差し替えは5行の書き換えで済む。**

★★★**2026-09-07 に参照画像の実測値へ差し替えた**（`docs/home-spec.md` §2。
`tools/extract-palette.py` で画素から測定。印字 HEX との差は最大 1）。

| 役 | 色 | 地 `#FCF7F4` の上 | 墨を載せて |
|---|---|---|---|
| **2つ目の明るい面**（中段のピル） | CREAM `#FEE1D1` | 1.17 | 12.33 |
| **提案 ── 2枚目以降** | SKY `#C3BFE0` | 1.67 | 8.64 |
| **提案 ── 1枚目** ／ タイケン | STORM `#858AC2` | 3.08 | 4.68 |
| **急ぎ** ／ 未読のバッジ ／ 危険 ／ バショ | ROSE `#E15442` | 3.56 | **4.05 ★** |
| **選択・リンク** ／ ジョウホウ | SEA `#144474` | **9.37** | 1.54 |

★★★**この5色は「役の階段」であって「分類の色」ではない。** 実測 …

| 尺度 | 2026-09-07 | 第80巡 | 意味 |
|---|---|---|---|
| 色相の最小隔たり | **5.4°** | 25.3° | 円を均等に覆うか |
| 明度の幅（L\*） | **63.2** | 44.2 | 暗・中・明がそろうか |
| 見分け（最小 ΔE） | 24.2 | 34.7 | 山の中で別の色に見えるか |

SKY / STORM / SEA は**同じ色相の3段**（296°／291°／275°）である。だから:

- ★★★**分類を色で言わない。** タグ5色は**廃止**し、タスクの図形は**濃さ5段**
  （`TAG_STEPS`）で分ける。見分けは**濃さ ＋ 書体**が持つ。
- ★★★**色が出るのは3か所だけ** … **帯の上段**（提案・急ぎ）／**山の提案の丸**／
  **未読のバッジ**。タブバー・タスクの四角・帯の中下段は**白と墨**
  （`docs/home-spec.md` §2-c）。★これで**骨格が色から独立する**。
- ★★★**文章が書ける色は SEA だけ**（9.37）。ROSE 3.56・STORM 3.08 は
  **面・縁・印**まで。SKY 1.67・CREAM 1.17 は**面だけ**。
- ★**ROSE に載せる和文は `lead`(16)／`WEIGHT` 700 以上**（比 4.05 で AA を割る。
  `docs/home-spec.md` §2-b。★目盛りの外なので、その行に理由を書く）。
...
- ★★★**「大きな文字の色」に使えるかは、地の上で決まる。** 実測 …
  SEA 9.37 ／ ROSE 3.56 ／ STORM 3.08 ／ **SKY 1.67 ✗** ／ CREAM 1.17 ✗。
  **薄い色は、面なら見えるが字にすると消える。**
- ★**目盛りの外** … 券の「モノ」（ドメインの4つ目）は SKY 1.67 で、大きな文字の
  3.0 に届かない。**新しい5色で 3.0 を満たすのは3つだけ**なので、4つ目は必ず割れる。
  ★Explore（券）は保留中で、再開するときに設計ごと見直す。
- ★★★**赤は「文章」にならない**。ROSE は地の上 3.56 ＝ **図形の 3.0** は満たすが
  **本文の 4.5 は満たさない**。赤が持てるのは**面・縁・印**だけで、
  エラーの文章は墨で書く。
- ★★★**盤の赤は1つになった**（2026-09-07）。新しい5色に赤は ROSE しか無いので、
  `redOn(面)` は面によらず ROSE を返す。**関数は残す** ―― 赤が2つに戻ったときに
  直す場所を1つに保つため。REC の輪も同じ ROSE（「光ったと分かるか」で選ぶ、
  という第79巡の選び方は変わっていない）。
- ★★★**明るい面にアクセントを直接置かない ―― 暗い窓の中で灯らせる**（第79巡）。
  白いキーの面の上では盤の色が出ないが、墨の窓 `STUDIO.socket` の中なら
  CREAM 12.33 ／ SKY 8.64 ／ ROSE 4.05 と全部出る。
- ★★★**押せる／押せないを面の色（グレーアウト）で言わない**（第79巡）。
  **沈み**（形）＝いま働いているか、**ランプ**（光）＝いま押せるか。
- ★★★**区切りに線を引かない**（第79巡）。`SectionLabel` ＋ 面のカード ＋ 余白。
  残ってよい `border` は**押せるものの縁**だけ。
- ★**地は3アプリとも `#FCF7F4` の1色**。暗いのは**機械**（録音の大きな
