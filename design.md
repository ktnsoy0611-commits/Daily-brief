...
★★**分類に使う有彩色は9つだけ**（第73巡に決め、第74巡に色を差し替えた）。
色そのものは `lib/constants.ts` の **`SCHEME`**（地と、その上に載せる文字の組）、
「役 → 色」は **`lib/palette.ts`** の1か所。**この2つ以外に色を書かない。**

| 役 | 地 | 文字 | 比 |
|---|---|---|---|
| ドメイン バショ | `#F9E595` | 黒 | 13.8 |
| ドメイン タイケン | `#FEAE8F` | 黒 | 9.7 |
| ドメイン ジョウホウ | `#B3D4E9` | 黒 | 11.2 |
| ドメイン モノ | `#D5BFC0` | 黒 | 10.0 |
| タグ WORK | `#03686D` | 白 | 6.6 |
| タグ LIFE ／ **肯定** | `#538352` | 白 | 4.4 |
| タグ WELLNESS | `#B3D2C5` | 黒 | 10.7 |
| タグ SOCIAL ／ **危険** | `#FE6C43` | 黒 | 6.2 |
| タグ GROWTH ／ **選ばれている** | `#8688C8` | 黒 | 5.3 |

- ★**状態の3色は9つから借りる**。新しい色を足さない。**ドメインの4色は
  どの状態とも重ならない**ので、券の色が UI の合図と混ざらない。
- ★地は `BD_GREY`(`#E1E1F0`) と `JOURNAL_BG`(`#CDBBA4`) の2つだけ。
