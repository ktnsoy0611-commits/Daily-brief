...
- ★**地は3アプリともクリーム `#FFFBF5` の1色**。★★★**録音の大きな円は
  第93巡に「黒い機械」をやめ、JOURNAL のメインカラー**（`JOURNAL_FACE`）へ
  （ユーザー指定「円の色をメインカラーに統一」）。明るい画面でも暗い
  オーバーレイでも**同じ色**。実測 … 地との比はクリーム **2.02**／墨 **5.9**。
  **明るい画面では円の輪郭が弱くなる**が、これは淡いアクセントをクリームに
  置く以上どうにもならない（`lib/appAccent.ts` に同じ記述がある）。
