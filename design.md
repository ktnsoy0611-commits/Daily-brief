...
- ★★★**タグの「柄」もこの規則の外**（第91巡）。べた塗り／網点は**質感ではなく塗り**
  ―― 分類を読ませるために**わざと絵を変えている**。`lib/tagPattern.ts`。
  ★ただし**網点の機械の規則は守る** … タイルは**デバイス画素**で作り、
  `pattern.setTransform(1/dpr)` で **1タイル画素＝1デバイス画素**に戻す。
  ★★図形は**回る**ので刻みは粗め（**12px**・点の半径 3.2）。細かいと回転の
  再標本化でモアレが出る（回転はこの節の想定外）。★**直径 ÷ 刻み ＝ 0.53** を
  保てば、大きくしても**面の被覆率＝色の濃さは変わらない**。
  ★CSS 側のぼかしは `DOT_R × 1.15` と**比例**で持つ（固定値だと点を大きくした
  途端に canvas 側より硬く見える）。
...
| **柄とグリフの刻み** | `lib/tagPattern.ts` の `DOT_PITCH`(12)／`DOT_R`(3.2)、`lib/textFit.ts` の `BUCKETS`(32/64/96/144/192)。★どちらも**画素の刻み**で、4の倍数へ丸めると柄が歪むか字が拡大される |
