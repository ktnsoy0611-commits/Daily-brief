...
- ★★**キーボードが引っ込んだら入力画面も閉じる**（2026-08-18 にユーザー指定
  ...
  合図は `--kb` を作っている追従の中で見る（120px 以上出たことがある → 60px を
  下回った）。**日程のシートが開いている間は通す**（あそこはわざと閉じている）。
...
- ★★**器の中に「はみ出し」を作らないこと**（2026-08-18・恒久）。
  ...
  そこを送ってくる。図形の canvas のように器より大きいものを置くときは
  **`overflow: clip`**（箱そのものを作らない）で切る。
  検証は「`[data-composer-shell]` の `scrollHeight - clientHeight` が 0」。
...
- ★★**日程だけはキーボードを閉じる**（`components/tasks/WhenSheet.tsx`。
  ...
  - ★★**時刻はタイムラインで範囲を選ぶ**（`components/tasks/TimeRange.tsx`。
    2026-08-18 にユーザー確定。ダイアル＝時と分の 2 本の輪は廃止した）。
    ダイアルは「いま何時か」は言えても「**どれくらいの長さか**」を見せられない。
    タイムラインなら長さがそのまま帯の長さになる。
    - **1 時間 = 44px**、見えるのは 200px（≒4.5 時間）。上下に送って時間帯を変える。
    - 帯をつまむと**動く**、上下の端をつまむと**伸び縮みする**。5 分刻み・最短 15 分。
    - ★**既定は 1 時間**（同指定）。時刻がまだ無い状態で開いたら、その場で
      09:00–10:00 を置く（つまむものが無い面にしない）。
    - ★**期日のときも同じ**（同確定）。期日／期間で操作が変わらない。
    - ★指が動いているあいだ **React を回さない**（ダイアルで踏んだ落とし穴）。
      目盛りは `useMemo` で作り置き、帯だけ ref 越しに style を書く。
      親へ返すのは 5 分の刻みをまたいだ瞬間だけ。
  ...
  - ★**中身が小さいホバーは幅も絞る**（`Float` の `fit`。2026-08-18 指定
    ...
    **相手の中心へ寄せ**、シートの縁 4px の手前で止める。タイムラインは 180px。
...
- `components/tabs/GravityTab.tsx`（落として積む）/ `DriftTab.tsx`（円環の
  カバーフロー）。
  ★★**画面の端では「切り落とす」のではなく「溶かす」**（2026-08-18）。
  「左右が見切れる」の直し方を一度間違えた — 図形と半径を縮めて画面に収めたが、
  求められていたのは**境目が直線で切れているのを直すこと**だった
  （「半径を小さくしてはみ出さないようにして欲しいわけではない」）。
  輪なのだから隣は画面の外へ回っていくのが正しい。
  `overflow: hidden` はそのままに、**横方向の `mask-image`**（`.drift-ring`）で
  端を薄れさせる。手前の 1 枚（画面の中央 300px ＝ 左右に 11.5% ずつの余白）には
  掛からない幅にすること。これで**大きさも半径も画面幅に縛られない**。
