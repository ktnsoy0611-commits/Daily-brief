...
- ★★**キーボードは絶対に閉じない**（実機で 3 度差し戻された）。閉じると器の
  高さが変わり、ボタンの位置ごと飛ぶ。引き金は「押したボタンへフォーカスが
  移ること」で、`onMouseDown` の `preventDefault`（フォーカス移動を止める定石）は
  **Chromium では効くが iOS Safari では取りこぼす** — iOS は `touchend` の時点で
  フォーカスを動かすことがあり、合成 `mousedown` を止めても間に合わない。
  **入力画面のボタンは例外なく `press(fn)` で押す**（`Popover.tsx` が持つ）:

  ```ts
  export const press = (fn: () => void) => ({
    onPointerDown: (e) => { e.preventDefault(); fn(); },  // click を待たない
    tabIndex: -1,                                          // 受け取らせない
  });
  ```
...
- ★**ポップオーバーを閉じたら必ず行へキャレットを戻す**（`backToRow`）。
  メモ・持ち物の欄は開くと自分にフォーカスを取るので、戻し先を指定しないと
  閉じた瞬間にフォーカスが `body` へ落ちてキーボードが消える。**閉じる道は
  3 つある**（✕ / 背面板 / ツールバーの丸をもう一度）ので、どれも同じ処理を通す。
...
- **日程は `components/tasks/WhenSheet.tsx`**（2026-08-17・参照＝TickTick）。
  `[ 期日 | 期間 ]` の切り替え。期日はカレンダー＋時刻の行（押すとダイアルが
  浮く／✕で終日）。期間は「終日」のスイッチで、ON なら開始・終了の日、
  OFF なら期日＋開始/終了の時刻。下に「クリア」。
  ...
  200ms 級のひっかかり。4× 絞りで 856ms → 767ms）。
  - ★**指の軸は最初の 8px で決める**。`pointerdown` した時点で横取りしていた
    ため、縦や斜めの指でも月が動いていた。縦と決まった指はその場で捨てる
    （カレンダーに縦へ送るものは無いので `touchAction: "none"`）。
  - ★**送ったあとに逆へ動かさない**。隣まで滑らせたら、その位置で `cursor` を
    進めて `slide` を 0 に戻すのだが、transition が生きていると中央へ戻る動き
    （＝逆走）が見えてしまう。**戻す 1 フレームだけ transition を切る**
    （`snapBack`。`.app-track` の再センタリングと同じ手）。
  - ★**曜日の行は送らない**（月ごとに描くと、送っている途中の並びが
    「T W T F S S M」のようにずれて見えた）。
  - ★**1 週の高さは残った高さから決める**（`ResizeObserver` で盤を測り
    `floor(h / 6)` を 18〜30px に丸める）。日のマスを 28px 固定にしていたため、
    キーボードで器が 470px まで縮んだうえ 6 週ある月では最後の週が切れていた。
    **常に 6 週ぶん描く**（5 週の月と混ざると送るたびに盤の高さが跳ね、下の
    時刻の行まで動く）。数字の大きさは丸の 0.62 倍で別に守る（比例させると
    9px 以下になって読めない）。
  - ★**ダイアルは真ん中に来た値がその時刻**（Apple のピッカーと同じ。
    2026-08-17 指定。以前は回したあとタップが要った）。`scroll` を 120ms
    デバウンス＋`scrollend` で `round(scrollTop / 34)` を確定する。
    幅は 154px・中央寄せ（全幅に広げない）。
...
- ★★**`document.fonts` には「先頭の family だけ」を渡す。**
  ...
  **本物の断片を取りに行かない**。実測（2026-08-17）:
  ...
  「赤と黄が同じに見える」ことになった）。`primaryFamily()` / `fontReady()` /
  `ensureGlyphs()` が唯一の窓口。`ctx.font` の方はスタックのままでよい
...
- **書体は待たずに「取りに行く」。** 断片の取得が始まるのは**その文字を
  ...
  購読者へ知らせる。
- ★★**書体が届くまで文字を描かない**（`computeTextPlan` が `fontReady` で
  門番をする）。グリフは 1 フレームに 1 枚しか焼かない（落下のガクつき対策）
  ので、焼いている途中で書体が届くと**前半 fallback・後半 本物**の絵になる
  （実機で「1 つの図形に 2 書体」として報告された）。キーに `ready` を混ぜる
  回避も試したが、キャッシュを捨てる順番に依存して脆かった。
  **fallback で焼かなければ混ざった絵は存在し得ない。** 図形だけ先に出て、
  届いた瞬間に全文字まとめて出る。実測: フォント取得を塞ぐと文字の面積 0%、
  塞がなければ 13.5%。
