...
- ★**キーボードは閉じない**。日付・重要度・タグを開いても出したまま
  （閉じると器の高さが変わり、ボタンの位置ごと飛ぶ）。ボタンを押すと
  フォーカスがそこへ移るだけでも iOS はキーボードを引っ込めるので、
  下の帯とポップオーバーの `onMouseDown` で**既定動作を止める**
  （`keepKeyboard`。文字を打つ欄だけ通す）。閉じるのは確定・完了・閉じるだけ。
...
- ポップオーバーは共通の器（`Popover`）＋中身。**地は墨**（2026-08-17 確定。
  ...
  **色の円**を 5 つ。高さはキーボードを出したままでも収まるよう、呼び側が
  測って `maxHeight` で渡す。
