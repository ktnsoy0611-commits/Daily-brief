...
**アイソメも 3D も持たない。どちらのビューも断面の形をベタ塗りで描くだけ。**

| ビュー | 形 | 載る文字 |
|---|---|---|
| FRONT | 断面の形。**四角と三角だけ**題の長さで横に伸びる。スラブの切れ目が入る | **タスクのタイトル**（中央） |
| BOTTOM | 断面の形。その形の自然な縦横比のまま | **タグの英字**（図形いっぱい） |

★以前は FRONT を長方形にしていたが、それだと FRONT だけで情報が完結し、
**「埋まっている側面の数 → 形」が何も意味しなくなる**（2026-08-16 のユーザー
指摘）。両ビューとも形で描くことで、形が両方の見え方で効く。

**書体は図形（タスク）ごとに 1 つ**（2026-08-16 確定）。1 文字ずつ変えるのは
明確に否定された。`tagFace()` がタグから `FONT_FACES` の番号を引く
（同じタグ＝同じ書体）。同じ図形の中の同じ文字はピクセル単位で同一になる。
...
（2026-08-16 確定）。明度から白黒を選ぶやり方（`inkOn`）は削除した — 相方の
色は参照画像が決めている。
...
| `SOCIAL` | `red` `#EE1B33` | `#F9C3C9` | 明朝 700 |
| `GROWTH` | `orange` `#FA6E31` | `#1B2B4F` | 明朝 400 斜体 |
...
### 寸法の対応
```
塗られる面積 = 重要度  … weightArea(小1.6/中3.6/大8.0) × (1 + 切迫度 × 0.6)
                        ★大きさに効くのはこれだけ。面積比 1 : 2.25 : 5
                        形ごとの塗り率(inkRatio)で外接箱を広げるので、
                        **形が違っても同じ重要度なら色の量が同じ**
                        （円/半円 0.78・三角 0.50・四角 1.00）
縦横比       … 四角と三角だけ、題の長さのはしご RATIOS へスナップ
                1:2 / 3:4 / 1:1 / 3:2 / 5:2 / 4:1（2文字以下 … 15文字以上）
                円は 1:1、半円は 2:1 のまま（伸びない）
形           … 埋まっている側面の数（1..4）        lib/solid.ts
スラブ       … 残っている手順（大きさには効かない）
色・書体     … タグ                                lib/taskTags.ts
落ちる順     … 切迫度（期日）
```
★`sectionOutline` は**縦横とも −0.5〜0.5** に正規化してある。こうしておくと
`(w, h)` を掛けた外接箱がちょうど `w × h` になり、「箱の面積 × 塗り率 ＝
塗られる面積」が形によらず成立する。その形の本来の比は `naturalRatio()` が別に持つ。

### タイトルの組み方
**「一定の大きさ」も「できるだけ大きく」も廃止**（2026-08-16）。
`layoutText()` が **`max(16px, 0.34 × √面積 × unit)`**（上限 64px）から始め、
幅で折り返し（最大 3 行）、それでも入らなければ縮める。置くのは
`innerBox` の**中央**。BOTTOM のタグ名だけは `fitText()` で図形いっぱい。
★基準に √面積 を使うのは**縦横比に左右されないため**。高さを基準にすると、
同じ重要度でも平たい帯だけ文字が小さくなる。

### 一括スケールと LOD
山が画面からはみ出さないよう、**全図形を一括で拡大縮小**する。
```
scale = clamp(√( (w × 床までの高さ) / UNIT² × FILL(0.58) ÷ Σ面積 ), 0.28, 1.6)
```
数個しか無いときは 1 を超えて拡大し、増えると縮む。**スケールが 0.02 より
動いたら山を作り直す**（当たり判定も同時に変わるため、絵だけ縮めると崩れる）。

LOD は**その形の代表寸法 `s = √面積 × unit`** で決める。
| `s` | 描くもの |
|---|---|
| `>= 48` | タイトル最大 3 行 ＋ スラブの切れ目 |
| `30〜48` | タイトル 1 行のみ |
| `< 30` | 文字なし（BOTTOM のタグ名も同じ） |
| `< 16` | 切れ目も描かない |
...
- **当たり判定は「そのビューで実際に描く形」**。どちらのビューも断面の多角形
