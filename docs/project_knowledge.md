...
**書体もタグと一対一**（2026-08-16 確定）。同じタグのタスクは必ず同じ書体で、
色と書体の 2 チャンネルでタグが読める。`faceIndexFor`（id から引く）は廃止し、
`tagFace()` が `FONT_FACES` の番号を返す。
...
### 寸法の対応（2026-08-16 に作り直し）
```
面積 = 重要度   … weightArea(小1.6/中3.6/大8.0) × (1 + 切迫度 × 0.6)
                  ★大きさに効くのはこれだけ。面積比 1 : 2.25 : 5
横幅 len        … タイトルの長さ。LEN_MAX=4.0（UNIT64 で 256px ＝
                  390px 画面の約 2/3）で頭打ち。超えたぶんは折り返す
高さ 2r         … 面積 ÷ 横幅
断面 section    … √面積（FRONT が平たくても BOTTOM は痩せない）
断面の形        … 埋まっている側面の数（1..4）    lib/solid.ts
スラブ          … 残っている手順（大きさには効かない）
色・書体        … タグ                            lib/taskTags.ts
落ちる順        … 切迫度（期日）
```
★アスペクトは 0.75〜3.2 に収める。そのとき動かすのは**高さではなく横幅**。
高さを押さえると面積が目標より膨らみ、「面積＝重要度」が崩れる（長い題の
中くらいのタスクが、大きいタスクに見えてしまう）。幅が足りないぶんは
タイトルの折り返しが受け持つ。

### タイトルの組み方
**「できるだけ大きく」は廃止**（2026-08-16）。`layoutText()` が
一定の大きさ（`TITLE_EM = 0.30 solid` ≒ UNIT64 で 19px）から始め、幅で折り返し
（最大 3 行）、それでも入らなければ入るまで縮める。置くのは図形の**左下**。
BOTTOM のタグ名だけは以前の指定どおり `fitText()` で**図形いっぱい**。
...
`paintShape` が**実際に描く px** から LOD を決める。
| 図形の高さ | 描くもの |
|---|---|
| `>= 46px` | タイトル最大 3 行 ＋ スラブの切れ目 |
| `28〜46px` | タイトル 1 行のみ |
| `< 28px` | 文字なし（BOTTOM のタグ名も同じ） |
| `< 14px` | 切れ目も描かない（ベタ塗り 1 枚） |
...
- **文字はグリフのアトラス経由で描く。** 和文の Web フォントは unicode-range で
  ...
  **(書体, 文字) の組ごとに初回 5〜10ms** かかる。素直に描くと落下中に
  ...
  書体をタグごと（5 種）にしてから (書体, 文字) の組み合わせがさらに減り、
  16 個の落下が 4× CPU 絞り・本番ビルドで**合計 819ms / 最大 171ms**
  （書体が図形ごとだった頃は 1186ms）。ビュー切替・積み終わったあと・
  横スワイプはいずれも 0ms。
