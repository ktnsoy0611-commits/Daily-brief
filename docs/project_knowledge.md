...
- ★★**キーボードが出ているあいだは `env(safe-area-inset-bottom)` を使わない。**
  ...
  帯がキーボードの真上に残る（2026-08-17 に「キーボードとアイコンの隙間が変」
  と報告）。器の矩形を書いている effect で
  `window.innerHeight - visualViewport.height > 80` を見て、帯の
  `padding-bottom` も **ref 経由で**書き分ける（setState は使わない）。
- ★★**日程だけはキーボードを閉じる**（`components/tasks/WhenSheet.tsx`。
  ...
  - ★**ダイアルは真ん中に来た値がその時刻**（iPhone のアラームと同じ。
    見える行は **5**）。`scroll` を 120ms デバウンス＋`scrollend` で確定する。
    幅は 162px・中央寄せ。**中の値は `<span>`**（上の恒久ルール 8）。
