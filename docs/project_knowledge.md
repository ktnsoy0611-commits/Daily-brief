...
（**iOS は和文の斜体を合成しない**ので黄の Dela と同じに見えた）→
**Reggae One** と 2 度替えた。**斜体で見分けを作らないこと。**
6 書体の実測の画素差（同じ字を 200px で焼いて比較）: Dela vs Reggae **21.9%**、
いちばん近い組（ゴシック 700 と丸ゴ 500）でも 8.8%。
...
| `SOCIAL` | `red` `#EE1B33` | `#F9C3C9` | Reggae One（極太） |
...
- ポップオーバーは共通の器（`Popover`）＋中身（`ComposerFields`）。日付は
  カレンダー（日は**円**・**横スワイプで月送り**）、重要度は**円の大きさ**、
  タグは**色の円**を 5 つ。高さはキーボードを出したままでも収まるよう、
  呼び側が測って `maxHeight` で渡す。
...
- **書体は待たずに「取りに行く」。** 断片の取得が始まるのは**その文字を
  ...
  購読者へ知らせる。
- ★**グリフのキーに「書体が届いているか」を混ぜる**（`glyphKey` の `ready`）。
  グリフは 1 フレームに 1 枚しか焼かない（落下のガクつき対策）ので、
  焼いている途中で書体が届くと**前半 fallback・後半 本物**の絵になる
  （実機で「1 つの図形に 2 書体」として報告された）。`ready` は
  `textPlanOf` が**一度だけ**決め、1 枚の絵の中では必ず一定にする。
