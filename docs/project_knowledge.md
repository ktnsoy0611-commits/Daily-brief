...
- **書体は待たずに「取りに行く」。** 和文は unicode-range の分割配信で、
  断片の取得が始まるのは**その文字を初めて描いた瞬間**。だから
  `document.fonts.ready` も `loadingdone` も「鳴る前に焼いてしまった絵」は
  救えず、**たまに前の書体のまま固まる**（実機で報告された）。
  焼く直前に `ensureGlyphs(face, text)` を呼び、その組が**まだ使えないとき
  だけ**、届いた時点でアトラスを捨てて購読者へ知らせる
  （`document.fonts.check` で先に見る。毎回捨てると描き直しが延々続く）。
