...
色はどちらもタグの色を全面に。文字は下地の輝度で白か黒、**1 文字ごとに
ランダム（決定的）な書体**。
...
- **物理の形は FRONT の長方形 1 つだけ。ビュー切替では body を作り直さない**
  （作り直すと山が崩れる）。形が変わったときだけ位置と速度を引き継いで差し替える。
...
- `lib/taskDemo.ts` と「デモのタスク／候補を入れる」ボタン
