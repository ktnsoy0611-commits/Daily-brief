...
## 直近で完了したこと（第59巡）— 出のフリーズ修正、誇張を 0.2 倍、閉じを繋ぐ

### ★★ALIGN の出が途中で止まって消えるバグ（写真2枚）
1. **原因** … 道の上の位置は `line = u - STREAM_GAP·i·conv` と**後ろへ引き戻して
   いる**のに、進み `u` を 1 で頭打ちにしていた。i 番目は `1 - 0.115·i` で**据わった
   まま**（i=8 なら 0.08 ＝ 道のほぼ出発点＝画面の真ん中）。そこへ引き渡しの時刻が
   来て円弧の入口へ飛ぶので「止まって、消える」。
2. **直し** … 1つずつ**自分の道の長さ** `span_i = 1 + STREAM_GAP·i` を持たせ、
   `u = cine(…/(STREAM_MS·span_i))·span_i`、引き渡しも `span_i` ぶん遅らせる。
   `join`/`conv` は `u/span_i` で見る。**どれも終点まで進んでから**渡る。

### 誇張を 0.2 倍へ（上品・洗練の方向）
3. `SQUASH_MAX` 0.55→**0.11** / `SMEAR_A` 0.34→**0.09** / `SMEAR_N` 3→**2** /
   `SMEAR_LEN` 0.9→**0.45** / `SMEAR_MIN` 9→**18**。速いときにだけ、ほのかに。

### ★★スクロールを `lib/scroll.ts` へ切り出し、効きを 0.2 倍
4. **新規 `lib/scroll.ts`** … 指 1:1 ＋ 投げ ＋ 減衰 ＋ 最寄りへ吸着。
   **強さは `SCROLL_GAIN`(0.2) と `FLICK_K`(0.18) の2つだけ**。ALIGN が使う。
   ★ユーザー指定「良かったら TIMELINE にも展開」なので最初から共通の部品。
   連鎖ばね（`CHAIN`）は据え置き ― 潰れて見えたのは**入力が強すぎた**から。

### TIMELINE
5. ★★**閉じるアニメーションを繋いだ** … `closeTimeline` が `setMode("pile")` を
   **即座に呼んでいた**ので、判定の瞬間に帯も縦線も消え「自由」も world から外れて
   いた。**`mode` は timeline のまま**にし、曜日は指に追従して潰れ、図形は落ち、
   「自由」も一緒に落として画面の下で畳む。全部が山へ戻ってから `pile` へ。
6. ★縦線は**離しても `ms(T_OUT)` は残る**／**土日はレーンに淡い地色**／
   **日付を `TYPE.head`(20)** へ。
7. ★★**「自由」が下まで落ちない**のは、板の**対角**がレーンの内寸を超えていて
   回ると壁に噛むから。**板は回さない**（`setInertia(Infinity)`）。字も大きく
   （`FREE_H` 0.62→0.78 / `FREE_SQUEEZE` 0.62→0.50）。
8. ★曜日を縦長に（`wdth` 75→**58**、`WD_ADV` 0.62→0.54 で 64px→**72px**）。

### 検証
`tsc`/`eslint`/機械チェック4本/本番ビルド ✓。`scratchpad/r59.mjs`（22項目）が
**4回連続で全部OK** — 出が止まらない／誇張の係数／スクロールの効き／閉じの 300ms 時点で
帯が残って潰れている／縦線の残存・土日・日付 20px／自由が床まで落ちきる／曜日 72px。
目視は `scratchpad/x9-*.png` と `y9-1..5.png`（出のスミアーが控えめになった図）。
★★**実機 Safari 未確認**。

### （参考）第52〜58巡の要点
第58巡: DRIFT の初回バグ（列が画面の外でマウントされる）を根治。掴んだら無重力。
的を「形そのもの」へ（唇と穴）。ALIGN のスクロールを連鎖に、スクアッシュ＆スミアーを
導入。TIMELINE に日付の縦線と閉じの段取り。
...
1. ★★**実機で確認してもらう**（第59巡ぶん）。**ALIGN の出**（途中で止まらないか／
   誇張が上品な量か）／**縦スワイプの効き**（払いすぎないか）／
   **TIMELINE の閉じ**（曜日が潰れながら図形が落ちるか）／**縦線**（残存・土日の
   地色・日付の大きさ）／**自由が床まで落ちるか**。
...
3. **触れる数字**（`GravityTab.tsx`）… 地面 `GROUND_LIFT`(32)・重力 `GRAVITY_Y`(1.4)、
   ...
   伸び `SQUASH_REF`(42)/`SQUASH_MAX`(0.11)、残像 `SMEAR_MIN`(18)/`_N`(2)/`_LEN`(0.45)/`_A`(0.09)。
   ...
   ★**スクロールの強さは `lib/scroll.ts` の `SCROLL_GAIN`(0.2)/`FLICK_K`(0.18) だけ**。
   ...
   `FREE_FILL`(0.90)/`FREE_H`(0.78)/`FREE_SQUEEZE`(0.50)。
...
- **完成時に撤去** … `lib/taskDemo.ts`「デモを入れる」ボタン、`ProfileTab` の
  「ブリーフ生成の実験」、「画面の数値を出す」（`lib/debugViewport.ts` /
  `components/tasks/ViewportProbe.tsx`）。
...
主な回帰（`scratchpad/`）… **`r59`（★出が止まらない・誇張の係数・スクロールの効き・
閉じの段取り・縦線と土日と日付・自由が床まで落ちる）/ `r58`（DRIFT の初回・無重力・
