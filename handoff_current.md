...
### 券は3巡目で確定。4巡目で形、5巡目で**向き**を直した

券（Vitsœ 型の色紙・題が主役・写真は余白をとった四角・紙のテクスチャは下地）と
鋏こん（一覧の語彙から アーチ／台形／角／二又・傾けて入鋏できる）は確定。

5巡目は**鋏の向き**。ユーザー指摘「今手前に向いているのは側面です」。
原因は**輪郭を画面座標で書いていた**こと ― 幅方向に短縮が無いので平面図が
そのまま立面図として読まれていた。押し出しを足しても面が立ち上がらない。

直したこと（詳細は `docs/explore-redesign.md` §2-b）:
- **工具の3軸 (l, w, h) で組んでから投影する**ようにした。`V` を `U` と
  直交させないことで幅が短縮され、天面が平行四辺形として立ち上がる。
- 頭を**2枚の板＋そのあいだの口**にした。板は軸の左右に振り分ける
  （重ねると1枚に見えて紙の入る隙間が読めない）。
- 天面に**ダイの出っ張り**、下の板に**受けのスリット**、手前の面に**口**。
- 一点透視（4巡目の性質）は `H` の横成分だけに残した。光と影は一定。
- 入鋏の瞬間は鋏を券の上に重ねるだけで「紙が口に入った」ように読める。
