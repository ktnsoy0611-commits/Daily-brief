...
### 券は3巡目で確定。6巡目で鋏の**軸の置き方**を直した

券（Vitsœ 型の色紙）と鋏こん（アーチ／台形／角／二又・傾けて入鋏できる）は確定。

6巡目は鋏。ユーザー指摘「Y軸に対して90度間違っている」。
**腕が開く向きを、地面と平行な軸に置いていた**のが誤り。実際はここが鉛直で、
だから参考写真（＝左面）をそのまま押し出した「横倒しのホチキス」になっていた。
**この向きでは紙を地面と平行に通せない** ― 機構として成立していなかった。

直したこと（詳細は `docs/explore-redesign.md` §2-b）:
- 3軸を (l, w, z) に取り直し、**`z` を鉛直**にした。腕はここで開く。
- 参考写真は `l–z` 断面そのもの。1本の背骨の **z の符号を反転させるだけ**で
  2本の腕になる（鋏は鋲で交差しているので符号の反転がそれを表す）。
- 口が**水平に開き**、紙が水平に入るようになった。
- ダイの窓は上顎の**天面**に開けた（下面だと上から見えない）。
- バネは `l–z` 面にあるので上から見ると細い楕円。これが正しい見え方。
- 一点透視（4巡目の性質）は `Z` の横成分に残した。光と影は一定。
