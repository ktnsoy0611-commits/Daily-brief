...
7. ★★**カメラのパンを「宇宙から地上へ降りる」大胆な動きにした**（追加指定）。
   - **層のあいだに空きを作った**（`LAYER_GAP` 0.4 → 移動量は画面の 140%）。
     間に何も無いと、どれだけ時間をかけても「切り替わった」に見える。
   - **`--t-cam`(1400ms) / `--ease-cam`(対称のS字) を語彙へ足した**。
     ★規約2つ（時間は5つ / 対称な ease-in-out は環境ループ以外で使わない）の
     **明示的な例外**として、理由付きで `:root`・`lib/motion.ts`・§3・§4 に
     記録した。**`TaskSpace.tsx` の外へ持ち出さないこと。**
   - **パン中だけ風（効果線）が流れる**（`.cam-wind`）。降りるときは上へ、
     昇るときは下へ。時間と曲線はカメラと同じものを引く。
   - ★踏んだ罠2つ … ①器の移動量だけ 140% にして `Layer` の間隔を 100% の
     ままにし、床がタブバーより 338px 高くなった（**必ず同じ数から作る**）。
     ②筋の長さを揃えたら端が一直線に並んで**カーテンに見えた**。

### 検証したこと

`tsc` / `eslint` / 本番ビルド ✓、機械チェック4本 ✓。
新規 `scratchpad/space38.mjs`（10群35項目）全部OK — 層の積み方と**あいだの
空き**・札が1枚でカメラの外・候補が重ならず浮く・タブでの上下・指での追従
（1:1は世界の側で測る）と吸着・行き止まりの抵抗・**床がタブバーの上端と一致**・
**パン中だけ風が正しい向きに流れる**・**時間と曲線が語彙から来ている**・
入力画面中はカメラが動かない。★**実機は未確認**。
