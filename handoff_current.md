...
作り、第33巡でデザインシステムを正規化した（寸法にも語彙を与え、機械で
検査できる形にした）。**第34巡で「下端 47px の帯」を解決した** — 第32巡に
「web からは直せない」と結論づけたのは誤りで、原因は
「`innerHeight` が上のセーフエリアぶん短いのに、中身は y=0 から描かれる」
ことだった（`--screen-h`）。

---

## 直近で完了したこと（第34巡）★下の帯（chin gap）を解決

**原因が分かった。** `black-translucent` ＋ `viewport-fit=cover` では、iOS は
**`innerHeight` を「画面の高さ − 上のセーフエリア」で計算するのに、中身は
画面のいちばん上（y=0）から描き始める。** 差のぶんが画面の下に「どの要素も
届かない帯」として残る。実測がぴたり一致 … **844 − 797 = 47 ＝ 上の
セーフエリア(47)**（下の 34 ではなく**上の 47**、が手がかりだった）。

- `--screen-h: calc(100% + env(safe-area-inset-top))` を語彙に足し、
  **全画面を塗る面だけ**その高さにした（`[data-paint]` を付けた7つ …
  帆布 / 作るものの輪 / ダッシュボード / 入力画面の板 / 録音の面 /
  読み込み中 / サインイン）。★**中身の置き場所には使わない** — タブバー・器・
  キーボード追従は 797 の中に収まっているのが正しい
- ★`statusBarStyle` は **`black-translucent` のまま**。`default`/`black` は
  下の帯こそ消えるが上に黒い帯が出て中身が下へずれる（第29巡に確認・差し戻し）。
  **`black-translucent` ＋ `--screen-h` なら上にも下にも帯が出ない**
- 検証 `scratchpad/chin34.mjs` を新設。Chromium はセーフエリアが 0 で再現
  できないので、**`--screen-h` を `calc(100% + 47px)` に差し替えて実機と同じ
  条件を作り**、塗る面が 47px 下まで届くこと・タブバーは伸びないことを見る
- ついでに**第33巡の積み残しのテスト2件**を直した … `rect24` は閉じる動きを
  「地の面の幅」で見ていた（第27巡から切り抜きの円が縮む形なので成り立たない）
  → 半径を見る形へ。`v6` は `locator.click()` が「動いていない」ことを待って
  時間切れになっていた → pointer 事象を直に撃つ形へ（`--t-out` を 350→600ms に
  してから顕在化していた）
- ★**`v7` はまだ落ちる**（第33巡からの積み残し・今回の変更とは無関係と
  stash して確認済み）。日程シートを開いたあと、器のキーボード判定が
  「閉じた」と誤解して入力画面ごと閉じているように見える。**実バグの可能性が
  あるので次に追うこと。**
...
1. **実機（iOS Safari）での確認**。★今回は「見た目の数字」を全面的に動かして
   ...
   - ★**タブバーの高さが 76 → 77px**（目印の行の余白を目盛りに乗せたため）。
     下部固定バーがタブバーへ潜っていないか。
   - ★★★**画面のいちばん下の帯が消えたか**（第34巡の本題）。作るものを選ぶ
     輪の円が下でまっすぐ切れないか／タブバーがホームインジケーターへ
     潜っていないか（潜っていたら `--screen-h` を置き場所へ使ってしまっている）。
...
- **実機 Safari の未検証** … 第33巡の見た目の変更全般、タスクアプリ全般、
  ジャーナルの円のドラッグとマイクの解放。
- ★画面のいちばん下 47px の帯は**第34巡で解決した**（`--screen-h`）。
  `statusBarStyle` は `black-translucent` のまま触らないこと
  （`default`/`black` は上に黒い帯が出て中身が下へずれる。第29巡に確認済み）。
