...
export type TasteInput = {
  ...
  emerging?: InterestSignal[]; // これから好みそうな傾向(分析が予測する、興味の広がりの狙いどころ)
  livingArea?: string;
};
...
<プロファイル>: ユーザーの関心を表す4つの信号
  ...
  これから好みそうな傾向: 分析から予測される、今後関心を持ちそうな方向(まだ確立していない・興味の広がりの狙いどころ)
...
export async function buildDeck(input: {
  ...
  const emergingSignals = (input.taste.emerging ?? []).filter((i) => i && typeof i.label === "string" && i.label.trim()).slice(0, 20);
