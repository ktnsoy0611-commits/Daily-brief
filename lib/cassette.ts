...
export const CASSETTE = {
  ...
  // ★★★**半径 3.6 と中心 7.6/16.4 は選んでいない。式から出る**（第97巡）。
  //   ユーザー指摘「**円が小さすぎる**」「**円の中心の距離が近すぎて、四角に対して
  //   真ん中に寄りすぎているように見えて気持ち悪い**」。第96巡の実測は
  //   左の縁 2.8 ／ **リール間 0.8** ／ 右の縁 2.8 ―― **間が縁の 1/3.5** で、
  //   2つの丸が真ん中に固まって見えていた。
  //   ★★★そこで「**本体の左の縁 ＝ リール間 ＝ 右の縁 ＝ 下の段までの隙間**」を
  //   条件にすると、`5.2 − R = (9.6 + R)/1.5 − 2R` の解が **R = 3.6 ちょうど**。
  //   4つの隙間が**全部 1.6** に揃う（上だけ 3.4 ＝ カセットのラベル面）。
  //   **目で選んだ数ではないので、次に誰が触っても同じ値へ戻る。**
  /** リールの丸2つ（**本体の縦の中心** y=12 を通る）。 */
  reels: [{ x: 7.6, y: 12 }, { x: 16.4, y: 12 }],
  reelR: 3.6,
