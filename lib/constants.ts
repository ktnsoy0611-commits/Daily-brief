...
// ── カラースキーム(2026-09-23・第127巡にユーザーが参照画像で指定) ──
//
// 参照画像 … Clustr の配色見本（4色が縦に並んだ版面）。
//
// ★★★**第127巡に「4色だけ」へ絞った**（ユーザー指定「**色を今度はこの画像の中の
//   四つのカラースキームにして欲しいです。タスクは赤、ジャーナルは黄色、提案は
//   オレンジにして、提案はジャンルごとの色分けは一旦無くして同じ色にしてください。
//   その他は青にしてください**」）。
//
// ★★★**色そのものはここだけ。パレットを替えるときは、この4行を書き換える。**
//   役との対応は下の `SCHEME`（と `lib/palette.ts`）にあり、呼び出し側は
//   名前しか見ていないので、4行を差し替えれば全部そのまま追従する。
//
// ★★★**「何であるか」を言う色は、もうアプリの4つだけ**（第126巡まではドメイン4を
//   色で分けていた）。**提案の形（`lib/cardShape.ts`）はそのまま残る** ――
//   ジャンルは**形が言い、色は言わない**。ユーザーの「一旦無くして」は撤回の余地を
//   残した言い方なので、`DOMAIN_COLOR`（`lib/palette.ts`）の表は**4行とも残して
//   同じ色を指している**。戻すときはその4行だけを書き換える。
//
// ★実測（**地 `#F7F6FB` との比** ／ **墨 `#2C2627` との比**）:
//   青 1.59 / 8.67 ／ 黄 1.01 / 13.98 ／ オレンジ 1.91 / 7.24 ／ 赤 2.89 / 4.77。
// ★★★**4色とも地とはほとんど明度差が無い**（1.01〜2.89）―― 参照画像もそうで、
//   **あの調和は明度差ではなく彩度差で立っている**（WCAG の比は明度しか見ないので、
//   このパレットは比で判断してはいけない）。★だから**地の上の小さな字にしない**。
//   面と、面の縁と、面の上の**大きな字**にだけ使う。
export const PALETTE = {
  azul:     "#AFC5FF",  // 青（Periwinkle）  … その他（ニュース・状態3）
  amarillo: "#FCFF82",  // 黄（Lemon）       … JOURNAL
  naranja:  "#FF9C6D",  // オレンジ（Cantaloupe） … 提案（EXPLORE。ドメイン4は全部これ）
  rojo:     "#FD5959",  // 赤（Watermelon）  … TASK ／危険
} as const;

// ★★★**役 → 色**。ここが「何がどの色か」の唯一の表。
//
// ★★★**キーの名前は変えない** ―― 呼び出し側が数十箇所で使っている。
//   タグ5は第93巡に廃止済みなので、`work`/`life`/`growth`/`wellness`/`social` は
//   もう分類ではなく「席」でしかない。**意味は右のコメントが持つ。**
//
// ★★★**`danger` だけは青にしない**（ユーザーの「その他は青」から外した1つ）――
//   削除やエラーの合図を青にすると、**押してよいもの（選ばれている＝青）と
//   取り返しのつかないものが同じ色**になる。赤は TASK と共有するが、
//   **危険が出る面（設定・削除の確認）に TASK の図形は出ない**ので衝突しない。
export const SCHEME = {
  ...
  danger: PALETTE.rojo,        // 赤       … 危険（★TASK と同じ赤。上の注釈）
} as const;

/**
 * ★★★**タスクの図形の色 ＝ 赤**（2026-09-23・第127巡にユーザー指定
 * 「**タスクは赤**」）。
 * ★★実測 … 地（`BD_GREY`）との比 **2.89**／**組の字（青）1.82**／墨 4.77。
 *   ★`bodyInkOn(TASK_FACE)` は**組の表**から青を返す（`lib/palette.ts`）。
 * ★★★**第117〜124巡の「タスクは無彩色」は撤回したまま**（`TASK_GREY` は残してある）。
 */
export const TASK_FACE = PALETTE.rojo;

/**
 * ★★★**ニュースの色 ＝ 青**（2026-09-23・第127巡。ユーザー指定の「**その他は青**」）。
 * ★★第125巡の「ニュースはあまり目立たないように」は**そのまま効いている** ――
 *   地との比 **1.59** は4色でいちばん低い側で、**縁の線としてもっとも退く**。
 */
export const NEWS_FACE = PALETTE.azul;

/**
 * ★★★**JOURNAL の顔の色 ＝ 黄**（2026-09-23・第127巡にユーザー指定
 * 「**ジャーナルは黄色**」）。録音の大きな円（ダイヤル）とホームの山のカセットの
 * 本体が同じ色を読む。
 * ★★実測 … 地との比 **1.01**（＝明度では地とまったく同じ。彩度だけで立つ）／
 *   暗いオーバーレイとの比 **13.98**／組の字（オレンジ）**1.93**。
 * ★★★**録音画面のラベルは組の表から導かれてオレンジになる**（比 1.93）――
 *   **参照画像どおりの組み合わせで、小さな字には厳しい**。ユーザー指定の上での選択。
 */
 ...
 * ★★★**第127巡に3つとも別の色へ**（前は `wellness`/`life`/`social` が
 * 黄・緑・ラベンダーだったが、`life` が青に移って `social` と黄で重なった）。
 * 墨の窓の上での比 … 黄 **13.98** ／ 青 **8.67** ／ オレンジ **7.24**。
 */
export const STUDIO_KEY = {
  pause: SCHEME.wellness,   // 黄       … 墨の窓の上で 13.98
  send: SCHEME.work,        // 青       … 8.67
  cancel: SCHEME.growth,    // オレンジ … 7.24
} as const;

// ★★**状態の色は3つだけ**（第73巡）。**分類の色から借りる**（専用の色を持たない）。
// ★★★**第127巡に `GREEN` も青になった**（ユーザー指定「その他は青」）。
//   **名前は変えない** ―― 13箇所以上が読んでいて、意味は「肯定・達成」のまま。
export const BLUE = SCHEME.work;     // 青  … 選ばれている・リンク（地の上 1.59）
export const RUST = SCHEME.danger;   // 赤  … 危険（削除・エラー）。★地の上 2.89
export const GREEN = SCHEME.life;    // 青  … 肯定・達成（★もう緑ではない）
/** ★★★**状態の面に載る色**（第76巡）。★★状態の色を**そのまま小さな文字に
 *  使わないこと** ―― 紙 `#FAFAF9` の上で 4.5 を満たす色は**1つも無い**
 *  （赤 2.98・青 1.64・オレンジ 1.97・黄 1.02）。**状態は「面」で見せる。**
 *  ★★★**`bodyInkOn()` と同じ結果を手で書いている**（`lib/palette.ts` を import
 *  すると循環参照になるため）。**パレットを替えたら必ずここも確かめる。**
 *  ★★★**第127巡から「組の表」を引く** ―― 青の面には赤、赤の面には青。 */
export const GREEN_INK = PALETTE.rojo;  // 青の面に載る（組。比 1.82）
export const BLUE_INK = PALETTE.rojo;   // 青の面に載る（組。比 1.82）
// ★第73巡に `PLUM` / `SLATE`、第74巡に `GOLD` を**消した**。`GOLD` は
//   「白い ✓ を載せる面」＝**肯定**そのものだったので `GREEN` に寄せた（13箇所）。
/** RUST の淡い敷き。★赤 `#FD5959` = rgb(253,89,89) から作る。 */
export const RUST_TINT = "rgba(253,89,89,0.14)";
export const RUST_EDGE = "rgba(253,89,89,0.50)";
/** GREEN の淡い敷き。★青 `#AFC5FF` = rgb(175,197,255) から作る。 */
export const GREEN_TINT = "rgba(175,197,255,0.24)";
