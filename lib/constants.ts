...
export const NIPPER_PAINT = {
   ...
   * この段に量子化する ― 手で塗らない。
   * ★★第70巡に**鈍い鉛色**へ寄せた（ユーザー指定・実物の写真が正）。
   *   それまではクラフト紙に合わせた暖色寄りの灰で、金属に見えなかった。
   *   明度の並びは変えず、色相だけ**わずかに寒色**へ振ってある。
   */
  ramp: ["#33353A", "#494C51", "#666A6F", "#8B9095", "#B2B7BC", "#D9DDE1"],
  /**
   * ★**光沢**（段の上に重ねる、なめらかなハイライト）。段だけだと平らな塗りに
   * 見えるので、金属らしさはここが作る。`shine` が大きいほど鋭い。
   */
  gloss: 0.55,
  shine: 26,
  /** ★**すり傷**の強さ（0 で無し）。段の境目を荒らし、光沢に筋を入れる。 */
  scratch: 0.5,
  /**
