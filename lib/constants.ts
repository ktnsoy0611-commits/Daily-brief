...
// ★★★**和文の書体の掛け金**（2026-09-20・第125巡にユーザー指定「**アプリ内の
//   日本語を全部これにしてみることはできますか？気に入らなかったら戻すので
//   戻せるようにはしてください**」）。
//   ★★★**戻すときは下の `JP` の右辺を `JP_NOTO` にするだけ。**
//     登録（`app/layout.tsx`）は両方とも残してあるので、他はどこも触らない。
//   ★★**欧文の並び（Archivo が先）は変えていない** ―― LINE Seed JP は欧文の
//     グリフも持つが、**「欧文だけ Archivo」はこの並び順が作っている**。
const JP_LINE = 'var(--font-line-seed), "LINE Seed JP"';
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- ★戻すための待機。消さない
const JP_NOTO = 'var(--font-noto-sans-jp), "Noto Sans JP"';
const JP: string = JP_LINE;
...
// ★★★**図形の字も `JP` を読む**（第125巡。ユーザー指定「**アプリ内の日本語を
//   全部これに**」）―― ここだけ Zen Kaku Gothic New に残すと、**帯のピルと
//   山の図形で書体が違う**という、いちばん目に付くずれになる。
//   ★戻すのは上の `JP` の1行（ここは触らない）。
const GOTHIC = `${JP}, var(--font-zen-kaku-gothic-new), "Hiragino Sans", sans-serif`;
...
// ── カラースキーム(2026-09-17・第117巡にユーザーが参照画像で指定) ──
//
// 参照画像 … Jardins de Métis（Reford Gardens）の入場券。**画素を測って**採った
// 7色（彩度のある画素を色相 × 明度でクラスタリングし、各塊の平均）。
//
// ★★★**大きな方針転換** ―― ユーザー指定
//   「**提案カード以外の色を無彩色にします。タスクに関する図形はグレーを主に使って
//   文字は黒で、つまりグレーの塗りに黒の字。提案カードやその他は画像の色を使って、
//   各ジャンルに割り当たっている図形の形は維持して、色とその形がセットで印象が
//   合うように。ジャーナルの図形は飛び出た色というか別枠なのが分かる配色関係に**」
// → **タスクは無彩色（`TASK_FACE` ＝ `MUTED`）／分類の色はドメイン4だけ／
//   JOURNAL はピンク1色で列の外**。
//
// ★★★**`lib/appAccent.ts`（アプリごとのアクセントのテスト）は第117巡に削除した。**
//   タスクが無彩色・JOURNAL がピンクになった時点で、あの試しの前提が消えた。
//   **復活させない。**
//
// ★★★**色そのものはここだけ。パレットを替えるときは、この8行を書き換える。**
//   役との対応は下の `SCHEME`（と `lib/palette.ts`）にあり、呼び出し側は
//   名前しか見ていないので、8行を差し替えれば全部そのまま追従する。
//
// ★★★**第125巡に総入れ替え**（2026-09-20・ユーザー指定「**今度はまたこの添付した
//   画像から色を拾ってカラースキームを構成して割り当ててください。タスクもニュースも
//   色を割り当てます。タスク、ジャーナル、提案の図形が主に目立つ色でバランスを
//   取ってください。ニュースは逆にあまり目立たない配色にしてください**」）。
//   **参照画像の画素を数えて、面積の大きい順に拾った8色。**
//
// ★★★**有彩色が要るのは 7** … **ドメイン 4** ＋ **JOURNAL** ＋ **TASK** ＋ **NEWS**。
//   8つ目の**赤は「危険」だけ**に使う（分類のどれとも重ならないので、削除や
//   エラーの赤がタスクや提案と見間違われない）。
//   ★★★**第117〜124巡の「タスクは無彩色」は撤回した**（`TASK_GREY` は残してある）。
//
// ★★★**赤をドメインから外した理由** ―― 参照画像の赤 `#E73115` とオレンジ
//   `#FD6A23` は**色相が 8° しか違わない**。オレンジをタスク（山にいちばん多く
//   出る図形）に当てる以上、同じ山に出るタイケンを赤にはできない。
//   → タイケンは**ピンク**へ。**赤は同じ画面に出ない役（危険）だけ**が持つ。
//
// ★★**「メイン × サブ」の組をやめた**（第75巡に導入）。面に載る字は
//   `bodyInkOn()` が**面から導く**（表を持たない ―― 表にすると、色を替えた人が
//   片方だけ直す）。
//
// ★実測（**地 `#FFFBF5` との比** ／ **墨 `#2C2627` との比**）:
//   緑 4.82 / 2.99 ／ ピンク 2.33 / 6.20 ／ ティール 2.70 / 5.33 ／ 黄 1.67 / 8.64 ／
//   ラベンダー 1.78 / 8.08 ／ オレンジ 2.80 / 5.14 ／ 水色 1.62 / 8.90 ／ 赤 4.20 / 3.43。
//   ★★**面に載る字は `bodyInkOn` が高いほうを選ぶ**ので、表を持たなくてよい
//     （緑と赤だけが紙、他は墨）。
//   ★★★**第125巡からピルは全部「輪郭だけ・黒の字」**（ユーザー指定）なので、
//     **地との比は「線の見えやすさ」だけに効く**。字はどのピルでも墨 15.5。
//     黄 1.67・ラベンダー 1.78・水色 1.62 の線は薄いが、**それが役どおり**
//     （水色＝ニュース＝退く）。**文字に使ってはいけない**のは今までどおり。
export const PALETTE = {
  verde:    "#387E41",  // 緑       … バショ（画像でいちばん面積が大きい）
  rosa:     "#FE7DAE",  // ピンク   … タイケン
  azul:     "#0AAAC1",  // ティール … ジョウホウ
  amarillo: "#FBBB04",  // 黄       … モノ
  lila:     "#D2B2F5",  // ラベンダー … JOURNAL（★列の外）
  naranja:  "#FD6A23",  // オレンジ … TASK
  cielo:    "#AECCE6",  // 水色     … ニュース（★唯一「退く」ための色）
  rojo:     "#E73115",  // 赤       … 危険だけ（★分類には使わない）
} as const;

// ★★★**役 → 色**。ここが「何がどの色か」の唯一の表。
//
// ★★★**色と形は組**（第117巡にユーザー指定）。ドメインの形（`lib/cardShape.ts`）と
//   印象が合うように選んだ:
//   バショ＝**四つ葉**（やわらかい弧）→ **緑**（土地・庭）
//   タイケン＝**六角形**（斜めの鋏痕）→ **赤**（券で面積が最大＝いちばん強い）
//   ジョウホウ＝**波打つ四角**（直角の鋏痕）→ **青**（知。前の Azul の役をそのまま）
//   モノ＝**トゲトゲ**（切れ込み）→ **黄**（バースト状の形と明るさが合う）
// ★★★**JOURNAL だけピンク** ―― 4つが赤・緑・青・黄の原色寄りの列を作るのに対し、
//   ピンクは**明らかにその列の外**。「別枠」を色相の距離ではなく**列に入らないこと**
//   で言っている。
// ★★**赤は1つしかないので `growth` と `danger` が同じ色**（前は Terracota と
//   Magenta の2本立てだった）。**危険はタイケンと同じ画面に出ない**ので衝突しない。
// ★★**タグ5は第93巡に廃止済み**なので、`wellness`/`social` は分類ではなく
//   「モノ」と「JOURNAL」の席になっている。**キーの名前は呼び出し側が数十箇所で
//   使っているので変えない。**
export const SCHEME = {
  work: PALETTE.azul,          // ティール … ジョウホウ(info) ／選ばれている
  life: PALETTE.verde,         // 緑       … バショ(place)    ／肯定
  growth: PALETTE.rosa,        // ピンク   … タイケン(experience)
  wellness: PALETTE.amarillo,  // 黄       … モノ(thing)
  social: PALETTE.lila,        // ラベンダー … JOURNAL（★別枠。ドメイン4には使わない）
  danger: PALETTE.rojo,        // 赤       … 危険（★分類のどれとも重ならない）
} as const;

/**
 * ★★★**タスクの図形の色は無彩色**（2026-09-17・第117巡にユーザー指定
 * 「**タスクに関する図形はグレーを主に使って文字は黒で、つまりグレーの塗りに
 * 黒の字**」）。**既にある `MUTED` を使う ―― 新しいグレーを増やさない**
 * （無彩色は5段で打ち止め）。
 * ★★実測 … 地（`BD_GREY`）との比 **3.2**／**黒い字 4.51**（AA）。
 *   ★`bodyInkOn(MUTED)` は `INK` を返すので、**面に載る字は自動で黒**になる。
...
/**
 * ★★★**ニュースの色 ＝ 水色**（2026-09-20・第125巡にユーザー指定「**ニュースは
 * 逆にあまり目立たない配色にしてください**」）。
 * ★★★**「退く」を彩度と明度で言う** ―― 参照画像の中でただ1つの**寒色の
 *   パステル**で、ドメイン4（緑・ピンク・ティール・黄）とタスク（オレンジ）と
 *   JOURNAL（ラベンダー）が作る**濃い列の外**に、下へ落ちる形で居る。
 * ★★実測 … 地との比 **1.62**／黒の字 **8.90**。
 *   **縁の線としては薄い**が、**それが「目立たない」の実体**（第125巡から
 *   ピルは全部 輪郭だけなので、色は 1px の線にしか出ない）。
 * ★★**グレーにはしない** ―― 無彩色はタスクが第117〜124巡に使っていた席で、
 *   「色を割り当てる」という指定に反する。**参照画像から拾った色で退かせる。**
 */
export const NEWS_FACE = PALETTE.cielo;

/**
 * ★★★**JOURNAL の顔の色 ＝ ピンク**（2026-09-17・第117巡にユーザー指定
 * 「**ジャーナルの図形も何かしらの色を与えるが、飛び出た色というか別枠なので
 * それが分かる配色関係に**」）。ドメイン4が赤・緑・青・黄の原色寄りの列を作る
 * 中で、ピンクだけが**その列の外**に居る。
 * ★★録音の大きな円（ダイヤル）とホームの山のカセットの本体が同じ色を読む。
 * ★★実測 … 地との比 **3.35**／暗いオーバーレイとの比 **3.5**／
 *   縁の目盛りは `bodyInkOn(この色)` ＝ 墨（**4.09**）。**半透明にしないこと。**
 */
export const JOURNAL_FACE = SCHEME.social;

/**
 * ★★★キーの記号の色（第78巡にユーザー指定「journal のボタン関係のアクセントに
 * 色をつけて」）。**盤のアクセント**を PAUSE / SEND / CANCEL に当てる。
 *
...
export const BLUE = SCHEME.work;     // 青  … 選ばれている・リンク（紙の上 3.26）
export const RUST = SCHEME.danger;   // 赤  … 危険（削除・エラー）。★紙の上 4.46
export const GREEN = SCHEME.life;    // 緑  … 肯定・達成（紙の上 3.90）
/** ★★★**状態の面に載る色**（第76巡）。★★状態の色を**そのまま小さな文字に
 *  使わないこと** ―― 紙 `#FAFAF9` の上で 4.5 を満たすのは `INK`(15.5) だけで、
 *  `RUST`(4.46)・`GREEN`(3.90)・`BLUE`(3.26) はどれも割る。**状態は「面」で見せる。**
 *  ★★★**`bodyInkOn()` と同じ結果を手で書いている**（`lib/palette.ts` を import
 *  すると循環参照になるため）。**パレットを替えたら必ずここも確かめる。** */
export const GREEN_INK = PAPER;  // 緑の面に載る（比 3.90。墨は 3.65 なので紙が勝つ）
export const BLUE_INK = INK;     // 青の面に載る（比 4.37。紙は 3.26）
// ★第73巡に `PLUM` / `SLATE`、第74巡に `GOLD` を**消した**。`GOLD` は
//   「白い ✓ を載せる面」＝**肯定**そのものだったので `GREEN` に寄せた（13箇所）。
/** RUST の淡い敷き。★赤 `#DB3321` = rgb(219,51,33) から作る。 */
export const RUST_TINT = "rgba(219,51,33,0.14)";
export const RUST_EDGE = "rgba(219,51,33,0.50)";
/** GREEN の淡い敷き。★緑 `#2B903E` = rgb(43,144,62) から作る。 */
export const GREEN_TINT = "rgba(43,144,62,0.24)";
 ...
 */
export const BAND_H = { photo: 60, plain: 44, news: 34 } as const;
