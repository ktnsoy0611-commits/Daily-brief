...
// ★★★墨は **`#23252B`**（2026-09-07・ホームの配色でユーザー指定。それまでは
// Velvet Black `#2C2627`）。地 `#FCF7F4` との比は **14.41** で、**前の組み合わせと
// 同じ値**（明暗の階層は動かない。移った先は色みだけ ―― 暖色 → 寒色）。
// ★★**差し替えたのは地と墨の2値だけ**。無彩色の段（`CHARCOAL` / `SECOND` /
// `MUTED` / `SURFACE`）と有彩色は**一切触っていない**（ユーザー指定）。
...
export const STUDIO = {
   ...
   *  ―― キーの輪郭は沈む穴の影だけで見せるので、面は地と同じ色でよい。 */
  cap: "#FCF7F4",
...
// ── カラースキーム(2026-08-31・第80巡にユーザーが参照画像で指定) ──
//
// ★★★**色そのものはここだけ。パレットを替えるときは、この6行を書き換える。**
//   役との対応は下の `SCHEME`（と `lib/palette.ts`）にあり、呼び出し側は
//   名前しか見ていないので、6行を差し替えれば全部そのまま追従する。
//
// ★★★**「9色要る」は思い込みだった**（第80巡に棚卸し）。要るのは **5**:
//   ・**タグ 5**（タスクの図形）… ここだけが「5色ないと足りない」。
//   ・ドメイン 4（券・鋏痕・マップ）… TASK と同じ画面に出ないので**タグと共有**。
//   ・状態 3（危険・肯定・選ばれている）… 上から**借りる**。専用の色を持たない。
//   9 は参照画像の色数で、要件ではなかった（予備の `lilac` と `sky` は
//   アプリ内で **0 回**しか使われていなかった）。
//
// ★★**「メイン × サブ」の組をやめた**（第75巡に導入）。サブは「色の面に載る
...
//   **6通りをすべて採点し、Magenta を外した組がいちばん良かった**
//   ―― 色相の最小隔たり **47.7°** ／ 明度幅 0.326 ／ 彩度幅 0.091 ／ 見分け 0.187
//   （前のパレットは 色相 28.0°・見分け 0.144）。
// ★★★**Magenta をタグから外したのが効いている** ―― Terracota と ΔE 0.162 で
 ...
 */
export const BAND_H = { photo: 70, plain: 52 } as const;

/**
 * ★★★**ホームの5色**（2026-09-07・ユーザー指定。参照画像の画素から測った値）。
 * ★★**既存の `PALETTE` / `SCHEME` とは別に持つ** ―― あちらは「分類の色」
 * （タグ・ドメイン・状態）の表で、こちらは**ホームの帯の5種類**という別の役。
 * 混ぜると、どちらかを替えたときにもう片方が意図せず動く。
 *
 * | 種類 | 色 | 地の上 | 載る字 |
 * |---|---|---|---|
 * | 1 今日入った提案 | STORM | 3.08 | 墨 4.68 |
 * | 2 今日行くのがおすすめ | SEA | 9.37 | 地の色 9.37 |
 * | 3 JOURNAL からの候補 | CREAM | 1.17 | 墨 12.33 |
 * | 4 フォローアップ | ROSE | 3.56 | 墨 **4.05 ★** |
 * | 5 期日未割当 | SKY | 1.67 | 墨 8.64 |
 *
 * ★★★**ROSE の 4.05 は本文の 4.5 に届かない。** 帯の文字を **16px / 700 以上**に
 * して「大きな文字 3.0」で通している（`components/home/Band.tsx`。ユーザー確定）。
 * ★役との対応表は `lib/homeBand.ts` の `BAND_FACE`。**色そのものはここだけ。**
 */
...
export const BD_GREY = "#FCF7F4";   // ★2026-09-07 にホームの配色の地へ（前は `#FFFBF5`）
