...
// ---- ブリーフのダミーデータ（14件、画像/情報ソース/地図色付き） --------
// フェーズ1はダミーデータのままデプロイする(実装引き継ぎドキュメント §8)。
// フェーズ2でGemini生成のブリーフに置き換わる。
// HANDOFF-CURRENT.md §8.2で確定した切り捨てに合わせ、FINANCE(貯金の進捗)・
// PHYSICAL(身体)カテゴリのカードは削除済み(貯金額・運動記録の入力箇所が
// アプリに存在せず、生成カテゴリからも除外する方針のため)。「ロケーション」
// トリガーのカードは、カレンダー連携が無い現状で成立する根拠(=ストックの
// ピン近接)に文言を揃えている。
export const CARDS: BriefCard[] = [
  ...
  { id: 2, glyph: "映", category: "CINEMA", categoryJp: "映画", trigger: "タイムリー", area: "日比谷", color: "#1A1712",
    ...
    body: "リスト登録済みの『Perfect Days 2』が昨日公開。日比谷のTOHOシネマズなら、金曜夜のレイトショー21:10の回にゆったり間に合います。",
  ...
  // 「ロケーション」トリガーの根拠は、カレンダー連携が無い現状では
  // 「あなたのストックに近いピンがある」という自データでのみ成立させる
  // (SYSTEM-DESIGN.md §該当箇所。以前は「土曜の外出ルートから」のように
  // 個人の予定を知っているかのような創作文言だったため修正した)。
  { id: 3, glyph: "珈", category: "NEIGHBORHOOD", categoryJp: "近所の発見", trigger: "ロケーション", area: "蔵前", color: "#33633F",
    ...
    title: "蔵前で見つけた、豆売りの焙煎所",
    body: "願望リストの「浅煎りの豆を買う」が叶えられそうなお店。あなたのストックにある蔵前エリアの候補と同じ並びにあります。土曜は焼き菓子の入荷日でもあります。",
    meta: ["蔵前・COFFEE WRIGHTS", "9:00 – 18:00", "ストックの蔵前ピンと近接"], bg: "#33633F", fg: "#F3ECDD", accent: "#A7C7AE",
  ...
  { id: 4, glyph: "陶", category: "SERENDIPITY", categoryJp: "未知との遭遇", trigger: "セレンディピティ", area: "清澄白河", color: "#7A4432",
    kind: "activity",
    title: "陶芸、はじめてみませんか",
  ...
  { id: 5, glyph: "古", category: "VINTAGE", categoryJp: "古着", trigger: "ロケーション", area: "高円寺", color: "#8A3C2A",
  ...
  { id: 6, glyph: "雑", category: "ZAKKA", categoryJp: "雑貨", trigger: "タイムリー", area: "谷根千", color: "#2C6E7A",
    kind: "thing",
  ...
  { id: 7, glyph: "本", category: "BOOK", categoryJp: "読書", trigger: "興味との一致", kind: "book", color: "#6B4A2E",
  ...
  { id: 8, glyph: "音", category: "MUSIC", categoryJp: "音楽", trigger: "登録サイトより", kind: "album", color: "#A67A2E",
  ...
  { id: 9, glyph: "劇", category: "LIVE", categoryJp: "音楽", trigger: "興味との一致", area: "下北沢", color: "#2A4A3A",
    kind: "live",
  ...
  { id: 10, glyph: "銭", category: "SERENDIPITY", categoryJp: "未知との遭遇", trigger: "ロケーション", area: "蔵前", color: "#3E5468",
  ...
  { id: 11, glyph: "映", category: "CINEMA", categoryJp: "映画", trigger: "興味との一致", area: "両国", color: "#4A3628",
    kind: "movie",
  ...
  { id: 12, glyph: "器", category: "ZAKKA", categoryJp: "雑貨", trigger: "ロケーション", area: "神保町", color: "#8A6B2E",
    kind: "exhibition",
  ...
  { id: 13, glyph: "本", category: "BOOK", categoryJp: "読書", trigger: "登録サイトより", kind: "book", color: "#2C5468",
  ...
  { id: 14, glyph: "陶", category: "VINTAGE", categoryJp: "古着", trigger: "タイムリー", area: "高円寺", color: "#8A3C34",
    kind: "thing", sourceWishTitle: "古着でジャケットを探す",
