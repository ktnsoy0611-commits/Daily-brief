...
//   **復活させない。**
//
// ★★★**色そのものはここだけ。パレットを替えるときは、この7行を書き換える。**
//   役との対応は下の `SCHEME`（と `lib/palette.ts`）にあり、呼び出し側は
//   名前しか見ていないので、7行を差し替えれば全部そのまま追従する。
//
// ★★★**有彩色が要るのは 5 だけ** … **ドメイン 4** ＋ **JOURNAL 1**。
//   状態3（危険・肯定・選ばれている）は上から**借りる**。
//   **タスクは有彩色を持たない**（第117巡に無彩色へ）。
//
// ★★**「メイン × サブ」の組をやめた**（第75巡に導入）。面に載る字は
//   `bodyInkOn()` が**面から導く**（表を持たない ―― 表にすると、色を替えた人が
//   片方だけ直す）。
//
// ★実測（**紙 `#FAFAF9` との比** ／ **墨 `#2C2627` との比**）:
//   赤 4.46 / 3.19 ／ 緑 3.90 / 3.65 ／ 青 3.26 / 4.37 ／ 黄 1.57 / 9.07 ／
//   ピンク 3.48 / 4.09 ／ ラベンダー 2.03 / 7.01 ／ 深緑 12.02 / 1.18。
//   ★★**面に載る字は `bodyInkOn` が高いほうを選ぶ**ので、表を持たなくてよい。
export const PALETTE = {
  rojo:     "#DB3321",  // 赤（券でいちばん面積が大きい）
  verde:    "#2B903E",  // 緑
  azul:     "#6D8CC1",  // 青
  amarillo: "#E3CB18",  // 黄
  rosa:     "#DD5591",  // ピンク（★この1色だけ列の外＝JOURNAL）
  lila:     "#B2AFCE",  // ラベンダー（予備）
  bosque:   "#083B2C",  // 深緑（予備。暗い面が要るとき）
} as const;
...
//   使っているので変えない。**
export const SCHEME = {
  work: PALETTE.azul,          // ジョウホウ(info) ／選ばれている
 ...
 * ・`news` … **ニュースの段**（2026-09-18・第122巡）。**面も縁も持たず字だけ**
 *   なので、いちばん薄い ―― 段が1つ増えたぶん**山の取り分が減る**ので、
 *   厚くしない（`buildPieces` は帯の下端を予算から引いている）。
 * ★3段の差が「上ほど自分に近いもの」を形で言っている。
 */
export const BAND_H = { photo: 60, plain: 44, news: 28 } as const;
