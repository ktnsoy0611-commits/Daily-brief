...
export const STUDIO = {
  // ★★★第78巡に**明暗がひっくり返った**（ユーザー指定）。
  //   ・journal（クリームの地）… **黒い機械**。`*Lit` の側。
  //   ・オーバーレイ（暗い地）… **白い機械**。`*Dim` の側。
  //   名前の `Dim`/`Lit` は**画面**を指す（`dim` ＝ 全画面のオーバーレイ）。
  //   ★どちらも「面 ＞ 穴」の順に暗くして、キーが**出っ張って**見えるようにする。
  /** オーバーレイの面（大きな円＋押せるキー）。★白い機械。 */
  figureDim: "#EFEAE8",
  /** journal の面。★黒い機械（地との比 11.25）。 */
  figureLit: "#3E3739",
  //  穴（キーが沈む所）。★必ず面より**離す**。ここが潰れると物として読めない。
  wellDim: "#8A8384",    wellLit: "#141112",
  //  押せないときの面。★「死んでいる」ように、地の側へ寄せる。
  capOffDim: "#C4BDBE",  capOffLit: "#6A6264",
  //  消えているランプ。★面の上で**かすかに見える**だけ（比 1.8〜2.3）。
  //  ★消えているランプは**面より明るい**側へ取る（黒い機械では 1.72／白い機械では
  //    2.28）。暗い側へ取ると穴と見分けがつかず、実測でも 1.42 しか出なかった。
  lampOffDim: "#A29B9C", lampOffLit: "#615A5A",
} as const;
...
//
// ★★**色の出どころはここ1つ**。9つの「**メイン × サブ**」の組（2026-08-31・
// 第75巡にユーザー指定の参照画像から。画像は「地の色に、その色専用の文字色で
// 大きく1語」を10枚並べたもので、**組で使う**という指定はそこから来ている）。
//   ・**メイン** … 面の色。これが普通の意味での「カラースキーム」。
//   ・**サブ**   … そのメインの上に載る**大きな文字**の色。メインごとに1つ決まる。
// ★★**サブは大きな文字（`head` 以上と `nano` の印）だけ**。本文（`body` 13）は
//   `INK`／`PAPER` を使う ―― 参照画像のサブは**どれも表示用の大きな字**で、
//   本文に要る 4.5 には届かない組がある（実測 2.98〜6.78）。載せる大きさで
//   要る比が変わるので、**役ごとに使い分けるのが正しい**（`lib/palette.ts`）。
//
// ★どの色をどの役へ当てるかは、**修正前（第73巡）の色との OKLab 距離が最小**に
//   なる組み合わせを総当たりで解いて決めた（9役・候補10組・ΔE 合計 1.243、
//   平均 0.138）。ユーザー指定「修正前の色に近いものを図形に割り当てる」の実装。
//   余った1組（サーモン `#F9BFB3` / `#9B1A1A`）は**使わない**。
//
// ★無彩色(INK / PAPER / BG / SHADE / MUTED / BD_GREY / JOURNAL_* など)は
// この置き換えの対象外。地とタイトルのグレースケールは今までどおり。
export interface ColorPair {
  /** ★**メイン**＝面そのものの色。これが「その色」の正体。 */
  main: string;
  /** ★**サブ**＝その面に載る**大きな文字**の色（券の題・印・罫、図形の名前）。 */
  sub: string;
}
export const SCHEME = {
 ...
 *
 * ★★★**同じ色を両方の面では使えない**（実測）… 杏・若草・桃は明るい面で
 * 1.2〜1.8 しかない。だから**この3色は暗い面のときだけ**乗せ、明るい面では
 * 記号を墨にする（`VoiceStudio` の `acc`）。
 * ★★**REC の赤はここに書かない** ―― `redOn()`（`lib/palette.ts`）が
 * 面から導く。乗る面が2つ（キーの面と地）あり、必要な赤が**逆になる**ため。
 */
export const STUDIO_KEY = {
  pause: SCHEME.sherbert.main,   // 杏   … 暗い面で 7.20
  send: SCHEME.limeade.main,     // 若草 … 9.45
  cancel: SCHEME.pink.main,      // 桃   … 6.35
} as const;
...
// 置かれた黒い機械**として読ませる。★`STUDIO.figureLit` と同じ値
// ―― 大きな円とキーの面は**同じ材料**（出どころを2つにしない）。
export const JOURNAL_FIG = STUDIO.figureLit;
// この地の上での控えめな文字色。
