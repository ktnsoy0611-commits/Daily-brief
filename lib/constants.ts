...
// 面は**明暗だけ**で分かれる。立体は「同じ輪郭をずらして暗い面で置く」押し出しで
// 作り、影も**ぼかさない1枚の面**。
// ★★★**見下ろしの絵**なので**上面がいちばん明るい**(第69巡4巡目)。
// 3巡目は中間調に寄っていて上面と側面の差が弱く、パースが伝わらなかった。
// face(上面) と side(側面) の差を広げてある ― **この差だけがパースを伝える**。
export const NIPPER_PAINT = {
  /** 稜線の光。上面の縁の面取りに当たる。★細い帯だけ。 */
  lit: "#EDEAE1",
  /** 上面。見下ろしているので、ここがいちばん広くて明るい。 */
  face: "#BFBCB2",
  /** 押し出した側面。★上面とはっきり差を付ける。 */
  side: "#6E6C65",
  /** さらに奥。受けの厚みなど。 */
  deep: "#4A4842",
  /** 受けのスリット(＝板を貫いた窓)。 */
