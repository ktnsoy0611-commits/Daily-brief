...
/** ★★ホームの「大きな英語」（今日の曜日）。地と `INK` の 13% の混色
 *  （`docs/home-spec.md` §2-d）。★**図形ではなく版面の柱**なので、地より
 *  ほんの少しだけ濃い ―― 図形と同じ濃さにすると、柱が図形として読まれる。
 *  ★`TAG_STEPS` のいちばん淡い段と同じ値になるが、**役が違う**ので別に持つ。 */
export const HOME_WORD = "#E0DCDA";
