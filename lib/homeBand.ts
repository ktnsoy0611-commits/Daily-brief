 ...
 * ★★★**ニュース（3段目）も輪郭**（2026-09-19・第123巡にユーザー指定
 *   「**ニュースもピルにしてください**」）―― **面を持たない**ので、上の2段
 *   （自分に関わるもの）より一段 静かに見える。★縁は墨（`INK`）でタスクのグレーと
 *   区別が付く。**絵は `components/home/NewsCard.tsx` の `NewsPill`。**
 */
export const isOutlined = (kind: BandKind): boolean =>
  BAND_ROW[kind] === 1 || kind === "news";
