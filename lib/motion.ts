...
/**
 * ★★★**2段階で開く**（2026-09-20・第125巡にユーザー指定
 * 「**押したらそのまま何の緩急もなくただ広がるだけだと面白くない。Google labo の
 * アニメーションは、最初にふわっとちょっと早く、回転しながら1／3くらい広がって、
 * そこで一回ゆっくりに回転しながらほんの少しだけとどまって、そこから一気にまた
 * 広がるという2段階のアニメーションになっているのでそれを再現してください。
 * また途中で止まる時もちょっといき過ぎて戻るような慣性をここでも入れてください**」）。
 *
 * ★★★**曲線も時間も増やしていない。** 曲線は `EASE_SETTLE` 1本のまま、時間は
 *   `T_IN` 1つのまま ―― **その中の割り振り（振り付け）だけ**がここに在る。
 *   ★★「曲線は4本・時間は5つ」の約束はそのまま。**5本目を作らない。**
 * ★★★**「とどまる」は等速** ―― 曲線を掛けると「止まっている」ではなく
 *   「また動き出した」に見える（帯の流れを `linear` にしているのと同じ理由）。
 * ★★★**慣性は「行き過ぎて戻る」で出す** ―― 1段目は `STAGE_P + STAGE_OVER` まで
 *   行き、とどまるあいだに `STAGE_P + STAGE_CREEP` へ**戻りながら**ほんの少し進む。
 *   ばねを1本足すのではなく、**行き先を少し先に置いて引き返す**だけ
 *   （`lib/spring.ts` を増やさないため）。
 * ★★**使うのは canvas と毎フレーム組み直すパスだけ**（`easeAt` と同じ約束）。
 */
/** 1段目で広がる量（＝「1／3くらい」）。★目盛りの外（振り付け）。 */
const STAGE_P = 1 / 3;
/** 1段目の終わりの行き過ぎ（＝慣性）。★目盛りの外（振り付け）。 */
const STAGE_OVER = 0.08;
/** とどまっているあいだに残る進み（＝「ほんの少しだけ」）。★目盛りの外（振り付け）。 */
const STAGE_CREEP = 0.03;
/** 時間の割り振り … 1段目 ／ とどまる ／ 2段目。合計 1。★目盛りの外（振り付け）。 */
const STAGE_A = 0.22;
const STAGE_H = 0.16;

export function twoStage(x: number): number {
  const t = Math.max(0, Math.min(1, x));
  const peak = STAGE_P + STAGE_OVER;
  // ① ふわっと、ちょっと早く ―― 行き先を少し先に置く。
  if (t < STAGE_A) return peak * easeAt(EASE_SETTLE, t / STAGE_A);
  // ② ゆっくり回りながら、ほんの少しだけとどまる（＝行き過ぎから等速で戻る）。
  const rest = STAGE_P + STAGE_CREEP;
  if (t < STAGE_A + STAGE_H) {
    return peak + (rest - peak) * ((t - STAGE_A) / STAGE_H);
  }
  // ③ そこから一気に。
  const u = (t - STAGE_A - STAGE_H) / (1 - STAGE_A - STAGE_H);
  return rest + (1 - rest) * easeAt(EASE_SETTLE, u);
}
