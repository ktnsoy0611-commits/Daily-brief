 ...
 * ★★第70巡に**ユーザー指定で 2 倍**にし、第72巡に**1.85 倍**へ戻した
 *   （「少し厚すぎた」）。図の 45／28 に対して 83／52。厚みの**比**（45:28）は
 *   保ってある。バネの z（`COIL_Z`）も同じ倍率で動かす（26 × 1.85 = 48）。
 * ★★**倍率を変えるときは HALF と COIL_Z を必ず同時に動かす**（片方だけだと
 *   バネが本体から浮く）。
