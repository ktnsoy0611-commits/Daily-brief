...
//
// ★★★**第127巡に有彩色は4つになった**（ユーザー指定）:
//   ・**TASK ＝ 赤**（危険も同じ赤）／**JOURNAL ＝ 黄**／**提案 ＝ オレンジ**／
//     **その他（NEWS・選択・肯定）＝ 青**。
//   ★★★**ドメイン4は色で分けない** ―― `DOMAIN_COLOR` の4行は全部オレンジで、
...
/**
 * ★★★**面 → その上の字の色は「組の表」が先に決める**（2026-09-23・第127巡に
 * ユーザー指定「**塗りと文字の組み合わせも添付画像に合わせて、図形の塗りと文字の
 * 組み合わせもそれにしてください**」）。
 *
 * ★★★**参照画像の組は「相互の対」2つだけ** … **青 ⇄ 赤** ／ **黄 ⇄ オレンジ**。
 *   どちらも**お互いを指している**ので片側だけでも足りるが、**4行そのまま書く**
 *   ―― 片側だけにすると、次に色を替える人が「残り半分は計算で出る」と思い込んで
 *   組を崩す。
 *
 * ★★★**この表はコントラストを満たさない**（実測 … 青の面に赤 **1.82** ／
 *   黄の面にオレンジ **1.93** ／ オレンジの面に黄 **1.93** ／ 赤の面に青 **1.82**）。
 *   参照画像は**版面いっぱいの大きな字**なのでそれで成立しているが、
 *   **11〜16px の本文では読めない**。ユーザー指定の上での選択で、
 *   **`design.md` §6 の「目盛りの外」として数字ごと記録してある。**
 *
 * ★★**4色以外は今までどおり計算で出す**（`INK` か `PAPER` の読めるほう）――
 *   地・墨・紙・面のグレーはこの組に入らないので、**判断は1つも増えない**。
 */
const PAIR: Record<string, string> = {
  [PALETTE.azul]: PALETTE.rojo,
  [PALETTE.rojo]: PALETTE.azul,
  [PALETTE.amarillo]: PALETTE.naranja,
  [PALETTE.naranja]: PALETTE.amarillo,
};
...
/** 面の色 → その上の本文の色。★**組の表が先**。無ければ比の高いほう（同点は `INK`）。 */
export const bodyInkOn = (main: string): string =>
  PAIR[main] ?? (contrast(main, INK) >= contrast(main, PAPER) ? INK : PAPER);

/**
 * ★★**その面で読める「赤の役」の色**（第127巡にパレットが替わって候補が変わった）。
 * 盤の赤は **`rojo` 1つ**しかないので、暗い面では**オレンジ**を代わりに返す
 * （実測 … 墨の上で 赤 **4.77** に対し **オレンジ 7.24**／明るい地の上は
 * 赤 **2.89**・オレンジ 1.91）。
 ...
 * してください**」）。**4行とも `SCHEME.growth`（オレンジ）を指す。**
 *
 ...
 *
 * ★実測 … オレンジは**白い紙の上で 1.97**（券の大きな英語に使うと、第126巡の
 *   最良 6.07 から大きく落ちる）。**目盛りの外**として `design.md` に記録した。
 */
export const DOMAIN_COLOR: Record<ItemDomain, string> = {
  place: SCHEME.growth,        // オレンジ … 四つ葉（やわらかい弧）
  experience: SCHEME.growth,   // オレンジ … 六角形（斜めの鋏痕）
  info: SCHEME.growth,         // オレンジ … 波打つ四角（直角の鋏痕）
  thing: SCHEME.growth,        // オレンジ … トゲトゲ（切れ込み）
};
