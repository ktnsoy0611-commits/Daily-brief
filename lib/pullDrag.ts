...
/**
 * ★★★**指が「横へ払った」と決まるまでの遊び**（px。第124巡に帯のスクロール、
 * 第126巡に**引き下ろしとの取り合い**へも使うようになった）。
 *
 * ★★★**判定は1つ**（`components/home/Band.tsx` の段と `Pill` と
 *   `components/home/NewsCard.tsx` の `NewsPill` が同じ数を読む）―― 別々に持つと、
 *   「帯は送り始めたのに、ピルはまだ引き下ろしのつもり」という**どちらつかずの
 *   フレーム**ができて、絵がちらつく。
 * ★★**小さくしないこと** ―― ピルは `touchAction: "none"` で縦も横も JS が受けるので、
 *   遊びが小さいと**下へ引く指の横揺れで帯が送れる**。
 * ★目盛りの外（手ざわり）。
 */
