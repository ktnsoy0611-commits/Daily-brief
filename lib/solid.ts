 ...
 *
 * ★★★**形は「幅の等しいピルを縦に積んだもの」**（第99巡 ―― 第115巡に S 字の
 *   折りたたみへ替えたが、**2026-09-17・第116巡にユーザー指定で積みへ戻した**
 *   「**やっぱり、図形の形はピルが積み上がったような形に戻してください**」）。
 *   ・段の高さ `rowH = 1 / n`、**角丸は `rowH/2`**（＝真のピル）。
 *   ・段は**隙間なく接する**ので、段と段の境目は縁が一度 `straight` まで戻る。
 * ★★**左右は対称**（折り返しが無いので）。だから `halfWidthAtStack` は
 *   `stackEdge` の値そのもので、**追加の包絡を取る必要が無い**。
 * ★★★**第115巡の S 字（`foldsAt`／`sideFolds`／2段ぶんを覆う弧）は削除した。
 *   復活させない。**
 */
