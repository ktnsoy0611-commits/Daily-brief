...
export function demoCandidates(now = new Date()): InboxCandidate[] {
  ...
  return [
    // 3面(円柱): 題だけ。空白の面が2つ残る。
    mk(1, { title: "自転車の空気を入れる", sourceText: "そういえば自転車の空気がだいぶ抜けてた" }),
    // 3面(円柱): 1つだけ埋まっている。
    mk(2, { title: "図書館に本を返す", when: "今週中", weight: 2, sourceText: "図書館の本、今週までだった気がする" }),
    // 4面(半円柱)。
    mk(3, {
      title: "歯医者を予約する", when: "来週のどこか", where: "近所の歯科", who: "ひとりで", weight: 2,
      sourceText: "歯医者、そろそろ行かないとまずい",
    }),
    // 5面(三角柱)。
    mk(4, {
      title: "母の誕生日を祝う", when: day(now, 21), where: "実家", who: "家族と", why: "節目の年だから", weight: 3,
      sourceText: "来月、母の誕生日。今年はちゃんとやりたい",
    }),
    // 6面(四角柱): 全部埋まっている。
    mk(5, {
      title: "金沢へ旅行", when: "来月の連休", where: "金沢", who: "ふたりで",
      why: "ずっと行きたかった", how: "新幹線で二泊", weight: 3,
...
export function demoTasks(now = new Date()): Task[] {
  ...
  return [
    // いちばん大きい: 重要 × 今日。
    mk(1, { title: "健康診断の予約", weight: 3, dueDate: day(now, 0), when: day(now, 0), where: "クリニック", why: "枠が埋まる前に" }),
    mk(2, { title: "家賃の振込", weight: 3, dueDate: day(now, 1), when: "明日まで" }),
    mk(3, { title: "定例会議の資料", weight: 2, dueDate: day(now, 2), when: "水曜の朝まで", where: "会議室A", who: "チームと" }),
    mk(4, { title: "クリーニングを取りに行く", weight: 1, dueDate: day(now, 5) }),
    // 重要だが期日は先。小さくなりすぎないことの確認になる。
    mk(5, { title: "確定申告の準備", weight: 3, dueDate: day(now, 60), when: "来年の2月", where: "自宅", how: "会計ソフトで" }),
    // いちばん小さい: 些末 × 期日なし。
    mk(6, { title: "電球を買う", weight: 1 }),
    mk(7, { title: "本棚を整理する", weight: 1, dueDate: day(now, 30) }),
  ];
}
