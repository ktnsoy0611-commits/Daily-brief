...
// ★図形の上に載せる文字。**1文字ごとにランダムな書体**を割り当て、与えられた
// 箱にできるだけ大きく収める(2026-08-13にユーザー指定。画像2のステッカーの
// ポスターのような見え方)。
//
// ★書体は「タスクの id と、その文字の位置」から決める。乱数を引くと再描画の
// たびに書体が変わってちらつくため、**必ず決定的に**選ぶこと。
//
...
/** その文字に割り当てる書体の番号。seed(タスクのid等)と位置から決まる。 */
export function faceIndexFor(seed: string, index: number): number {
  // 位置ごとに桁を大きく混ぜる(連番の index でも選ばれる書体が偏らないように)。
  const h = Math.imul(hashOf(seed) ^ Math.imul(index + 1, 2654435761), 2246822519) >>> 0;
  return h % FONT_FACES.length;
}

export const faceFor = (seed: string, index: number): FontFace => FONT_FACES[faceIndexFor(seed, index)];
...
export function missingGlyphs(text: string, seed: string, color: string): number {
  let n = 0;
  [...(text ?? "")].forEach((ch, i) => { if (!hasGlyph(faceIndexFor(seed, i), ch, color)) n++; });
  return n;
}
...
export function warmGlyphs(text: string, seed: string, color: string, budget: number): number {
  ...
  [...(text ?? "")].forEach((ch, i) => {
    if (left <= 0) return;
    const fi = faceIndexFor(seed, i);
    if (hasGlyph(fi, ch, color)) return;
...
/**
 * 箱 w×h に、1文字ずつ違う書体で組んだ文字列をできるだけ大きく収める。
...
export function fitText(text: string, seed: string, w: number, h: number, maxLines = 4): FitResult | null {
  ...
  const widths = chars.map((c, i) => advanceOf(faceIndexFor(seed, i), c));
...
export function drawFitted(
  ...
  lines.forEach((line, li) => {
    ...
    for (const ch of [...line.text]) {
      const fi = faceIndexFor(seed, index);
