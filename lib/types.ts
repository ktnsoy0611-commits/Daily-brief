...
// 持たない(同じ意味の値を二重に持たない)。
//
// ★★★**埋まっている数はもう形を選ばない**（2026-09-13・第99巡にユーザー確定）。
// 1=円 / 2=半円 / 3=三角 / 4=四角 の語彙は消えた。**復活させない。**
// いまここが担うのは **`isDated`（"due" が入っているか）＝塗りか輪郭か**だけ。
// 形は「ピルの積み」1つで、**段の数は題の行数**が決める（`lib/solid.ts`）。
...
export interface AppState {
  ...
  // ★★★ホームの帯の並びの記憶(2026-09-16・第114巡)。「この id は、この id の
  // 次に置く」という**留め金の列**で、`after` が空文字なら段の先頭。
  // ★★**なぜ「順番の配列」ではなく「留め金」なのか** ― 帯の中身は毎日入れ替わる
  // (提案が増え、タスクに日付が付いて消える)ので、全件の並びを持つと**次の日には
...
// ★★★2026-09-07 に**ホームが4つ目の列**として加わった。ホームは3アプリの
//   **入口**で、自分のタブを持たない ―― タブバーの中身が「3アプリの名前」に
//   変わるだけで、列の仕組み（横の無限循環）は何も足していない。
...
export interface VoiceControls {
  ...
  /** 各棒を測った時刻(録音の経過ms)。★描く側は本数を数えて等間隔に置くのでは
   *  なく、この時刻から実際の位置を出す。setInterval は 45ms ちょうどでは
   *  来ないので、等間隔に置くとそのズレがそのまま「ガクつき」になる。 */
  timesRef: { current: number[] };
